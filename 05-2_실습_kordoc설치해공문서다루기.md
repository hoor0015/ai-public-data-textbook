...
**과제.** 폴더의 파일에서 법인마다 총자산, 수익, 사업비용을 뽑아 한 표로 모으고, 그것을 집계해 다음을 담은 보고서를 과의 양식에 맞춰 hwpx 파일로 만든다. 법인 수와 세 항목의 평균과 중앙값, 항목별 상위 법인, 그리고 자료를 다루면서 발견한 문제점이다. 재료는 두 가지다. 자료 폴더 `data/gongik/`에는 월별 폴더 열두 개와 3,103개의 xls 파일이 있고, 같은 폴더의 `data/gongik/보고양식_공익법인분석.hwpx`가 과의 보고 양식이다. 양식은 개요, 현황, 문제점, 시사점, 붙임의 다섯 절로 되어 있고 현황 절에 표 두 개가 비어 있다. 이 과제는 수업 시간에 풀지 않는다. 수업에서는 아래의 실마리와 작성 기준을 안내하고, 각자 집에서 에이전트와 함께 완성해 과제 5-B로 낸다.
