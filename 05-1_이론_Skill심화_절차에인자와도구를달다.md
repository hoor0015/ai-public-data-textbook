...
라이브러리가 이 정도로 자라면 스킬 목록과 변경 기록을 담은 안내 문서 하나를 폴더에 함께 둔다. 어느 스킬이 어디에 사는지, 무엇이 언제 왜 바뀌었는지가 적혀 있어야 몇 달 뒤의 나와 팀원이 같은 라이브러리를 쓸 수 있다. 다음 회차 실습 2.6에서 기준선 비교와 오류 심기를 직접 해 보고, 2.7에서 스킬의 위치를 정하고 안내 문서와 변경 기록을 만들며 공유 전 점검을 수행한다.
