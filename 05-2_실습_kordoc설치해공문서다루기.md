...
1회차에서 남이 만든 스킬을 가져다 쓰는 길을 배웠다. 공개된 스킬이 어떤 모양으로 배포되는지(마켓플레이스와 플러그인), 설치하기 전에 무엇을 확인해야 하는지, 설치한 스킬은 어떤 이름으로 불리는지를 다루었다. 오늘은 그 길을 실제로 걸어 본다. 1회차에서 배운 개념은 그것이 필요한 단계의 "먼저 알아 둘 것"과 "짚고 가기"에서 화면에 나온 결과를 놓고 다시 짚는다. 오늘 할 일은 GitHub에 공개된 kordoc이라는 플러그인을 설치해서, 한글 공문서 파일을 에이전트가 읽을 수 있는 형태로 바꾸고 그 안의 표를 데이터로 옮기는 것이다.
...
진행은 세 단계다. 앞부분(2.1-2.3)은 설치다. 저장소를 먼저 살펴보고, 마켓플레이스를 등록해 플러그인을 설치하고, 설치된 스킬의 SKILL.md를 열어 4주차에 손으로 만든 파일과 같은 구조임을 확인한다. 가운데(2.4-2.5)는 사용이다. 예시 공문서를 Markdown으로 바꾸고, 그 안의 표를 CSV로 옮겨 pandas로 읽는다. 뒷부분(2.6-2.7)은 관찰과 검증이다. 스킬 이름을 부르지 않아도 에이전트가 스스로 꺼내 쓰는지 보고, 변환 결과가 원문과 같은지 사람이 직접 대조한다. 마지막의 2.8은 과제 안내다. 엑셀 서식 3,103장에서 법인별 재무 항목을 모아 과의 양식대로 hwpx 보고서를 만드는 과제를 소개하고, 수업에서는 풀 실마리와 작성 기준만 다룬다. 2.9는 각자 구한 문서로 같은 절차를 되풀이하는 선택 심화다.
...
- (과제 5-C) `data/gongik/`의 공시 서식 3,103장을 집계해 양식대로 만든 보고서 hwpx
...
| 데이터 | `data/docs/인구현황보고_예시.hwpx` (수업용으로 만든 가상의 예시 공문서. 실제 행정기관이 발행한 문서가 아니며, 문서 안의 수치는 수업에서 쓰는 공개 데이터에서 가져온 값이다), `data/sigungu_2023.csv` (2.7의 대조에 쓴다. 4주차와 같은 파일), `data/gongik/` (2.8의 과제 자료. 국세청 공익법인 결산서류 공시 표준서식 3,103장, 2020년 공시분, 사회복지 분야. 월별 폴더로 나뉘어 있다), `data/gongik/보고양식_공익법인분석.hwpx` (과제의 보고 양식. 수업용으로 만든 가상의 과 양식이다) |
...
**무엇을 왜 하는가.** 설치는 남이 쓴 파일을 내 컴퓨터로 가져와 내 에이전트가 따르게 하는 일이다. 그러니 설치 버튼을 누르기 전에 저장소를 먼저 본다. 확인할 것은 넷이다. 누가 만들었는가, 무엇을 하는가, 내 컴퓨터에서 무엇을 실행하는가, 라이선스는 무엇인가. 이 조사는 에이전트에게 시키고 학생은 그 요약을 저장소 화면과 대조한다. 조사를 대신 시켜도 되는 이유는, 조사 결과가 맞는지는 브라우저를 열어 바로 확인할 수 있기 때문이다. 확인할 수 있는 일은 맡기고, 확인은 사람이 한다.
...
다섯 번째 항목이 오늘 가장 중요한 줄이다. 이 스킬은 문서를 읽어 주는 지시문만 들어 있는 것이 아니라, 내 컴퓨터에서 프로그램 하나를 내려받아 실행한다. 무엇이 실행되는지 모른 채 설치하는 것과, npx로 kordoc이라는 변환 프로그램이 돈다는 것을 알고 설치하는 것은 다른 일이다.

**짚고 가기: 설치는 파일을 받는 일이 아니다.** 설치라는 말은 파일 몇 개를 내려받는 일처럼 들리지만 실제로 일어나는 일은 그보다 크다. 스킬을 설치한다는 것은 남이 쓴 절차를 내 에이전트가 따르도록 허락하는 일이고, 그 절차가 터미널 명령을 부른다면 남이 정한 코드가 내 컴퓨터에서 도는 일이다. 스킬 본문은 사람이 읽는 지시문이면서 동시에 에이전트가 따르는 명령이므로, 그 안에 파일을 지우거나 내 폴더의 내용을 바깥으로 보내는 절차가 적혀 있어도 겉보기에는 평범한 문서로 보인다. 외부 용역의 산출물을 검수 없이 업무에 넣지 않는 것과 같은 이치로, 만드는 쪽을 심사해 주는 곳이 따로 없으니 쓰는 사람이 확인한다.

![그림 5-2. 설치 전에 확인할 네 가지](figures/fig05_before_install.png)

그림 5-2의 네 칸이 방금 에이전트에게 시킨 조사의 뼈대이고, 칸마다 보는 위험의 종류가 다르다. 누가 만들었는가는 지속의 문제다. 오래 방치된 저장소는 문제가 생겨도 고쳐지지 않는다. 무엇을 하는가는 적합의 문제이고, 어떤 도구를 부르는가는 안전의 문제이며, 요약의 (5)번이 여기에 답한다. 라이선스는 권리의 문제다. 라이선스는 남의 창작물을 어떤 조건으로 쓸 수 있는지를 밝힌 약속이고, kordoc의 MIT는 조건이 느슨한 편이어서 저작권 표시만 남기면 쓰고 고치고 배포하는 것이 폭넓게 허용된다. 표시가 아예 없는 저장소는 수업이나 업무에 써도 되는지 알 수 없다. 넷 중 하나라도 답을 채우지 못했다면 아직 설치할 때가 아니다. (자세한 설명은 1회차 1.3, 표 5-2)

**검증.** 브라우저에서 저장소를 직접 열어 주소와 계정 이름의 철자, MIT 라이선스 표시, 그리고 `.claude-plugin/marketplace.json`과 `plugins/kordoc/skills/kordoc/SKILL.md` 두 파일이 실제로 있는지 확인한다. 비슷한 이름의 다른 저장소를 읽고 온 것이 아닌지 보는 것이다. 하나라도 어긋나면 설치하지 않고 교수자에게 알린다. 그리고 요약에 "확인 못 함"이 있다면 그것부터 사람이 직접 찾아본다. 모른다고 적어 준 항목은 잘못이 아니라 정직한 보고이므로, 그 자리를 채우는 것이 사람의 일이다.
...
**무엇을 왜 하는가.** 설치는 두 걸음이다. 먼저 저장소를 스킬 목록으로 등록하고(마켓플레이스 추가), 그다음 그 목록에서 꾸러미 하나를 고른다(플러그인 설치). 한 걸음이 아니라 두 걸음인 이유를 알고 쳐야 화면에 뜨는 응답을 읽을 수 있으므로, 명령을 입력하기 전에 두 낱말의 자리를 먼저 정해 둔다.

**먼저 알아 둘 것.** 마켓플레이스는 목록이고, 플러그인은 그 목록에 실려 있는 꾸러미다. 목록 하나에 꾸러미가 여럿 들어 있을 수 있고, 목록만 등록해 두고 아무것도 설치하지 않을 수도 있다. 그래서 등록과 설치가 각각 한 줄씩이다. 오늘 쓰는 kordoc은 목록 이름도 kordoc이고 그 안의 꾸러미도 kordoc 하나뿐이라 두 이름이 겹쳐 보이는데, 같은 낱말이 다른 자리에 쓰인 것일 뿐이다.
...
<strong>정의 · 플러그인과 마켓플레이스</strong><br>
<strong>플러그인</strong>(plugin)은 Claude Code에 얹어 쓰는 확장 꾸러미다. 스킬을 하나 이상 담을 수 있고, 이름과 버전을 가진다. <strong>마켓플레이스</strong>(marketplace)는 그런 플러그인들의 목록이며, GitHub 저장소 안에 <code>.claude-plugin/marketplace.json</code> 파일로 적혀 있다. 마켓플레이스를 한 번 등록해 두면 그 목록에 실린 플러그인을 이름으로 골라 설치할 수 있다. 상급기관이 내려보낸 서식 안내문이 마켓플레이스라면, 그 안내문에 실린 매뉴얼 한 벌이 플러그인이고, 그 매뉴얼에 들어 있는 절차 한 편이 스킬이다.
</div>

![그림 5-1. GitHub 저장소의 세 층과 내 컴퓨터에 설치되는 자리](figures/fig05_marketplace.png)

그림 5-1의 왼쪽 점선 상자가 2.1에서 브라우저로 들여다본 그 저장소다. 맨 위 칸이 방금 눈으로 확인한 `.claude-plugin/marketplace.json`이고, 그 아래 칸이 플러그인 꾸러미이며, 꾸러미 안에 스킬의 SKILL.md가 들어 있다. 가운데 화살표 둘에 적힌 명령이 곧 입력할 두 줄이다. 위 화살표가 1단계로 목록을 등록하는 일이고, 아래 화살표가 2단계로 꾸러미를 내려받는 일이다. 오른쪽 상자는 그 결과가 내 컴퓨터에 놓이는 자리이며, 경로에 마켓플레이스 이름과 플러그인 이름과 버전이 차례로 들어간다. 그 자리가 실제로 그렇게 생겼는지는 이 단계 끝의 검증에서 직접 확인한다. 여기서 알 수 있는 것은 명령 두 줄이 저장소에 층으로 쌓여 있던 것을 그대로 내 컴퓨터로 옮긴다는 점이고, 옮겨진 것은 내 원본이 아니라 특정 판의 사본이라는 점이다. (자세한 설명은 1회차 1.2, 표 5-1)
...
**검증.** 세 가지를 확인한다. 첫째, `/plugin` 관리 화면에서 Installed 탭에 kordoc이 있는지, Marketplaces 탭에 kordoc 목록이 등록되어 있는지, Errors 탭이 비어 있는지 본다. Errors 탭에 뭔가 적혀 있다면 설치 과정에서 문제가 있었다는 뜻이므로 내용을 읽어 본다. 둘째, 자동완성 목록에 `/kordoc:kordoc`이 뜨는지 본다. 뜨지 않으면 `/reload-plugins`를 한 번 더 입력하거나 Claude Code를 재시작한다. 셋째, 설치된 파일이 실제로 어디에 놓였는지 확인한다. 아래 지시로 확인할 수 있다.
...
에이전트가 알려 주는 경로는 사용자 홈 폴더의 `.claude/plugins/cache/` 아래에 마켓플레이스 이름, 플러그인 이름, 버전 순으로 이어진 폴더다. 그림 5-1의 오른쪽 상자가 가리킨 자리가 이것이다. 오늘의 경우 `.claude/plugins/cache/kordoc/kordoc/4.12.0/`과 같은 모양이 되며, 맨 뒤의 숫자는 설치한 시점의 판에 따라 다를 수 있다. 여기서 두 가지를 확인한다. 폴더가 프로젝트 폴더 안이 아니라 사용자 홈 폴더 아래에 있다는 것, 그리고 경로에 버전 번호가 들어 있다는 것이다. 앞의 사실은 이 스킬이 어느 프로젝트를 열든 따라온다는 뜻이고, 뒤의 사실은 나중에 새 판이 나오면 다른 폴더에 따로 놓인다는 뜻이다.
...
**짚고 가기: 가져다 쓸 것인가, 만들 것인가.** 표 5-5의 마지막 줄이 남의 스킬을 쓸 때의 핵심이다. 방금 열어 본 SKILL.md도 텍스트 파일이라 얼마든지 고칠 수 있지만, 그 파일이 놓인 폴더는 2.2에서 본 대로 설치 관리의 대상이어서 다음 판을 설치하면 고친 내용이 덮인다. 그러니 남의 스킬에 손대고 싶어지면 그 자리에서 고치는 대신 필요한 부분을 내 스킬로 따로 만드는 편이 낫다. 여기에 걸려 있는 것이 유지보수의 주체 문제다. 가져다 쓴 절차는 만든 이가 고쳐 나가므로 문서 형식이 바뀌어도 내가 할 일이 없다는 것이 이점이지만, 같은 이유로 어느 날 결과가 달라질 수 있다. 새 판에서 표를 옮기는 방식이 조금 바뀌면 지난달 산출물과 이번 달 산출물의 모양이 달라진다. 설치 경로에 버전 번호가 들어 있는 것이 이때 도움이 되므로, 결과가 결정적인 업무에서는 어느 판을 쓰고 있는지 적어 두고 판을 올린 뒤에 같은 파일을 다시 변환해 결과가 전과 같은지 확인한다. 반대로 만든 이가 손을 놓으면 그 절차는 그 자리에 멈춘다. 남에게 맡긴 것은 편해진 것이지 없어진 것이 아니다.

![그림 5-3. 가져다 쓸 것인가, 만들 것인가](figures/fig05_make_or_take.png)

그림 5-3이 그 판단의 갈래다. 맨 위에서 시작해 질문 세 개를 차례로 지나며, 각 질문에서 오른쪽으로 빠지면 그 자리에서 결론이 난다. 오늘 kordoc은 세 질문을 모두 통과해 맨 아래의 "가져다 쓴다"로 왔다. 같은 일을 하는 공개 스킬이 있었고(질문 1), 2.1의 네 가지 점검을 통과했으며(질문 2), 한글 문서를 Markdown으로 바꾸는 일에는 우리 기관만의 기준이 들어갈 것이 없다(질문 3). 여기서 알 수 있는 것은 실제 답이 둘이 아니라 셋이라는 점이다. 가운데 오른쪽의 "가져다 쓰고 얹는다"가 실무에서 가장 흔한 답이고, 방금 본 표 5-5의 마지막 줄이 바로 그 자리로 이어진다. 변환은 공개 스킬에 맡기고, 그렇게 얻은 표를 우리 과의 서식으로 정리하는 절차는 내가 만든 스킬에 담는 식이다. 오늘 2.5에서 변환된 표를 CSV로 옮긴 뒤 4주차에 만든 스킬로 진단해 보는 대목이 그 모양의 축소판이다. (자세한 설명은 1회차 1.5, 표 5-3)

**검증.** 세 가지를 확인한다. 첫째, 화면에 찍힌 파일 경로가 2.2에서 확인한 설치 폴더로 시작하는가. 프로젝트 폴더 안의 다른 파일을 읽어 온 것이라면 경로가 다를 것이다. 둘째, 머리말에 `name`과 `description`이 있고 `name`이 kordoc인가. 셋째, 4주차에 만든 `.claude/skills/csv-profile/SKILL.md`와 나란히 놓고 두 파일의 뼈대가 같은지 본다. 위쪽에 `---` 사이의 머리말이 있고 아래쪽에 사람이 읽을 수 있는 글이 있다는 점이 같아야 하며, 같다는 것을 확인하는 것이 이 단계의 목적이다.
...
**검증.** 두 가지를 확인한다. 첫째, `data/docs/인구현황보고_예시.md`가 실제로 저장되었는지 본다. 대화창에 내용이 보였다고 파일이 생긴 것은 아니다. 둘째, 문서의 뼈대가 온전한지 센다. 큰 제목 한 개, 작은 제목 네 개(개요, 시군구 인구 상위 5곳, 합계출산율, 붙임), 표 한 개와 그 데이터 행 다섯 줄, 그리고 마지막 줄의 "끝." 표시가 있어야 한다. 공문서는 마지막에 "끝."을 적는 관행이 있으므로, 이 표시가 보이면 문서가 중간에서 잘리지 않았다는 뜻이다. 값이 맞는지는 2.7에서 본격적으로 대조한다.
...
**검증.** `data/인구상위5.csv`에서 세 가지를 본다. 첫째, 파일이 여섯 줄인가. 열 이름 한 줄과 데이터 다섯 줄이어야 한다. 둘째, 마지막 데이터 줄이 `5,경기,성남시,922518`인가. 셋째, 총인구 값에 쉼표가 남아 있지 않은가. 쉼표가 남아 있으면 pandas가 그 열을 숫자가 아니라 문자열로 읽어, 합계를 내려고 할 때 숫자가 더해지지 않고 글자가 이어 붙는다. 실제로 그런 파일을 읽어 합계를 내면 `1,190,9641,076,5351,074,971…`처럼 값이 줄줄이 이어진 결과가 나온다. 4주차에서 "숫자여야 할 열의 자료형이 문자열이면 숫자 아닌 값이 섞여 있다는 뜻"이라고 한 이상 신호가 이 모양으로 나타난다.
...
**짚고 가기: 스킬이 늘어나면 판단할 거리도 늘어난다.** 방금 에이전트가 한 일은 고르는 일이었다. 에이전트는 평소에 스킬의 본문까지 들고 있는 것이 아니라 이름과 설명만 목록으로 갖고 있다가, 요청이 들어오면 그 설명들과 요청을 맞춰 보고 무엇을 꺼낼지 정한다. 지금 이 프로젝트의 목록에는 4주차에 만든 csv-profile과 방금 설치한 kordoc 둘이 올라 있다. 방금 요청에 kordoc이 나섰다면 그 설명이 요청과 맞아떨어진 것이고, 나서지 않았다면 맞아떨어지지 않은 것이다. 문제는 이 목록이 학기가 갈수록 길어진다는 점이다. 설치가 한 줄이라 가벼운 만큼 시험 삼아 설치한 꾸러미가 쌓이기 쉽고, 설명이 서로 비슷한 스킬이 여럿이면 엉뚱한 것이 나설 여지가 생긴다. 서랍에 든 매뉴얼이 스무 권이면 필요한 것을 꺼내기 어려워지는 것과 같다.

표 5-4에 지우는 명령과 별개로 끄는 명령이 있는 이유가 여기에 있다. `/plugin disable`은 꾸러미를 지우지 않고 목록에서 잠시 내려놓는 일이고 `/plugin enable`로 다시 올린다. 지금 학기에 쓰지 않는 꾸러미는 꺼 두고, 아주 지우기로 마음먹었더라도 먼저 꺼 두고 한동안 지켜보는 편이 낫다. 오늘 설치한 kordoc은 앞으로도 계속 쓰므로 켜 둔 채로 간다. (자세한 설명은 1회차 1.4)
...
**짚고 가기: 잘 만들어진 도구일수록 안을 들여다보지 않게 된다.** 방금 나온 대조표는 다섯 줄이 모두 "일치"였다. 이런 표를 몇 번 받고 나면 다음부터는 마지막 열만 훑고 넘어가게 되는데, 4주차에서 자동화 편향이라는 이름으로 다룬 경향이 그것이다. 남이 만든 스킬을 쓸 때는 이 경향이 한 겹 더해진다. 내가 적은 절차라면 어느 단계에서 무엇을 하는지 알고 있으므로 결과가 이상할 때 짚을 곳이 있지만, 오늘의 변환 절차는 내가 적은 것이 아니다. 2.3에서 설치된 SKILL.md를 열어 본 것도 절차를 외우려던 것이 아니라 이상할 때 들여다볼 자리를 미리 확인해 둔 것이다. 대응은 지난 주들과 다르지 않다. 도구를 의심하는 대신 결정적인 값을 원문과 대조하는 것이고, 오늘의 경우 그 값은 인구 다섯 개와 출산율 세 개다. 여덟 개를 확인하는 데 1분이면 된다. (자세한 설명은 1회차 1.5)

**검증.** 대조표도 결국 에이전트가 만든 것이므로 사람이 표본 하나를 확인한다. `data/sigungu_2023.csv`의 31번째 줄에서 성남시의 총인구가 922518인지 보고, 같은 값이 변환된 md 파일과 CSV 파일에도 그대로 있는지 본다. 원본 데이터, 변환된 문서, 옮겨 적은 CSV의 셋이 한 줄로 이어지면 오늘의 옮겨 적기는 통과다.

한 가지 정직하게 짚어 둘 것이 있다. 이 대조가 확인해 주는 것은 "문서에 적힌 숫자가 데이터와 같다"는 사실이지 "변환이 원문을 하나도 빠뜨리지 않았다"는 사실이 아니다. 원문에 있었는데 변환에서 통째로 사라진 부분이 있다면 이 대조로는 잡히지 않는다. 조용히 사라지는 것들이 정해져 있으니 알아 두면 좋다. 표가 아니라 그림으로 들어 있는 통계, 각주에 달린 단서, 여러 쪽에 걸쳐 잘린 표가 그렇다. 셋 다 오류 메시지가 뜨지 않으므로 결과만 보아서는 알 수 없고 원문을 열어야 드러난다. 사라진 것을 잡는 일은 2.4에서 한 구조 세기(제목 네 개, 표 하나, 마지막의 "끝." 표시)가 일부 맡고, 한글 프로그램이나 한컴독스로 원문을 열어 볼 수 있는 학생은 원문 화면과 변환 결과를 직접 견주어 보면 가장 확실하다. 검증에는 이렇게 각각 잡아내는 것이 다른 여러 방법이 있고, 어느 하나가 전부를 대신하지 못한다.
...
## 2.8 과제 안내: 엑셀 서식 3,103장을 보고서 한 장으로

**시나리오.** 이번 과제는 가상의 상황에서 시작한다. 여러분은 어느 구청 사회복지과의 인턴이고, 오늘 아침 과장님이 폴더 하나를 건넸다. 안에는 엑셀 파일 3,103개가 월별 폴더 열두 개에 나뉘어 들어 있다. 파일 하나하나는 공익법인이 국세청에 낸 결산서류 공시 서식(상속세 및 증여세법 시행규칙 별지 제31호서식)이고, 2020년 한 해 동안 공시된 사회복지 분야 법인들의 것이다. 과장님의 말은 짧다. "이거 정리해서 우리 관내 법인 지원 사업 기획할 때 참고하게, 법인들이 대체로 어느 규모인지, 큰 데는 어디인지, 자료에 문제는 없는지 보고서로 한 장 만들어 줘. 양식은 우리 과 것 쓰고." 그리고 과장님은 회의에 들어갔다. 이 과장님과 구청은 수업을 위해 지어낸 것이지만, 폴더 안의 파일은 실제 공시 자료이고 이런 부탁은 실제 행정에서 흔하다.

**과제.** 폴더의 파일에서 법인마다 총자산, 수익, 사업비용을 뽑아 한 표로 모으고, 그것을 집계해 다음을 담은 보고서를 과의 양식에 맞춰 hwpx 파일로 만든다. 법인 수와 세 항목의 평균과 중앙값, 항목별 상위 법인, 그리고 자료를 다루면서 발견한 문제점이다. 재료는 두 가지다. 자료 폴더 `data/gongik/`에는 월별 폴더 열두 개와 3,103개의 xls 파일이 있고, 같은 폴더의 `data/gongik/보고양식_공익법인분석.hwpx`가 과의 보고 양식이다. 양식은 개요, 현황, 문제점, 시사점, 붙임의 다섯 절로 되어 있고 현황 절에 표 두 개가 비어 있다. 이 과제는 수업 시간에 풀지 않는다. 수업에서는 아래의 실마리와 작성 기준을 안내하고, 각자 집에서 에이전트와 함께 완성해 과제 5-C로 낸다.

**이 과제가 5주차에 있는 이유.** 오늘 앞에서 한 일은 문서 한 장을 읽어 표 하나를 데이터로 옮기는 것이었다. 이 과제는 같은 일을 3,103장에 대해 하고, 거기서 나온 숫자를 다시 문서로 되돌린다. 읽는 방향과 쓰는 방향이 모두 들어 있고, 어느 쪽도 손으로는 할 수 없는 규모다. 1회차에서 정리한 판단, 곧 남이 만든 도구를 가져다 쓰고 그 위에 내 절차를 얹는다는 것이 여기서 처음으로 실제 크기의 일이 된다. 파일을 읽고 쓰는 일은 공개된 스킬에 맡기고, 어느 칸의 값을 어떤 기준으로 모을지, 무엇을 문제로 볼지는 여러분이 정한다.

**실마리 1. 서식은 데이터가 아니다.** 파일을 하나 열어 보면 값이 행과 열로 정리되어 있지 않다. 사람이 읽는 서식이라 "18총자산가액", "31총계", "42소계" 같은 이름표가 붙은 칸 옆에 숫자가 놓여 있고, 그 칸의 위치는 서식 판에 따라 다르다. 그래서 몇 번째 행 몇 번째 열을 읽으라고 하면 어느 파일에서는 맞고 어느 파일에서는 엉뚱한 값이 온다. 값을 칸의 위치가 아니라 칸의 이름표로 찾게 하는 것이 첫 실마리다. 총자산은 "2. 재무현황"의 총계 행에서, 수익은 "4. 수익현황"의 총계 행에서, 사업비용은 "5. 비용현황"의 총계 행에 있는 사업비용 소계에서 읽는다. 어느 칸을 골랐는지는 보고서의 개요에 적어야 한다.

**실마리 2. 3,103장이 다 같은 서식이 아니다.** 파일 이름은 모두 "표준서식"이지만 서식에는 판이 있다. 대부분은 2019년 개정 서식이고, 100개 안팎은 그보다 앞선 판이어서 절의 이름과 항목 번호가 다르다. 2019년 판의 "2. 재무현황"이 앞선 판에는 "2. 자산보유현황"으로 되어 있고 수익과 비용의 항목 구성도 다르다. 이름표로 찾게 하면 이 파일들은 값이 비어 나오고, 그것이 곧 발견이다. 두 판을 억지로 한 표에 넣지 말고, 몇 개인지 세어 개요와 문제점에 적고 집계에서 어떻게 다루었는지 밝힌다. 에이전트가 이 차이를 알아채지 못하고 3,103개를 모두 집계했다고 보고한다면, 빈칸의 개수를 물어보는 것이 그것을 잡는 방법이다.

**실마리 3. 오래된 엑셀 형식.** 파일의 확장자가 xlsx가 아니라 xls다. 3주차에 만든 환경에는 이 오래된 형식을 읽는 부품이 없어서, 에이전트가 첫 파일을 여는 순간 오류가 난다. 3주차 2.6에서 보았듯 필요한 패키지는 그때그때 더하면 되고, 어떤 부품을 더했는지는 설계도(pyproject.toml)에 남는다. 오류가 나면 무엇이 빠졌는지 읽고 더하게 하면 된다. 이것은 일부러 심은 함정이 아니라 오래된 행정 자료를 다룰 때 늘 만나는 장면이다.

**실마리 4. 같은 법인이 두 번 나온다.** 파일 3,103개가 법인 3,103곳은 아니다. 한 법인이 2018년 사업연도와 2019년 사업연도의 공시를 각각 낸 경우가 있고, 같은 해 공시를 다시 낸 경우도 있다. 법인명으로 세어 보면 두 번 이상 나오는 이름이 400곳을 넘는다. 이것을 모른 채 상위 법인 표를 만들면 같은 재단이 1위와 2위에 나란히 오른다. 한 법인에 하나의 공시만 남기는 기준을 정해야 하는데, 어느 것을 남길지(가장 최근 사업연도인지, 가장 나중에 낸 것인지)는 여러분이 정하고 개요에 적는다.

**실마리 5. 평균이 대표값이 아닐 수 있다.** 세 항목 모두 평균이 중앙값보다 몇 배나 크다. 국민연금공단처럼 수익이 수십조 원인 곳이 이 자료에 섞여 있어서, 한 곳이 평균을 끌어올린다. 과장님이 물은 것은 "법인들이 대체로 어느 규모인가"이므로 평균만 적으면 답이 틀린다. 평균과 중앙값을 나란히 놓고 왜 다른지 한 문장을 붙이는 것이 현황 절의 요체이고, 상위 몇 곳이 전체의 몇 %를 차지하는지가 그 근거가 된다. 10주차에서 배울 분포의 치우침을 여기서 먼저 만나는 셈이다.

**실마리 6. 앞뒤가 맞지 않는 값.** 서식에는 총자산이 부채와 순자산의 합이라고 적혀 있다. 그런데 이 자료에서는 그 등식이 맞지 않는 법인이 넷에 하나꼴이다. 부채와 순자산을 모두 0으로 둔 곳, 순자산에 총자산과 같은 값을 그대로 적은 곳이 많다. 이런 것은 오류 메시지가 나지 않고 표도 반듯하기 때문에 확인해 보지 않으면 모른다. 등식 하나를 검산 항목으로 두고 몇 곳이 어긋나는지 세면 문제점 절의 가장 좋은 재료가 된다. 어긋난 값을 고쳐서는 안 된다. 원본은 그대로 두고 몇 곳이 왜 어긋나는지 적는 것이 여러분의 일이다.

**실마리 7. 도구는 자유롭게 고른다.** 오늘 설치한 kordoc에는 문서를 읽는 기능 말고도 Markdown 글을 공문서 모양의 hwpx로 만들어 주는 기능이 있고, 보고서와 계획서 같은 프리셋이 있다. 양식 파일을 kordoc으로 읽으면 다섯 절의 짜임과 표의 열 이름이 그대로 보이므로, 그 짜임대로 Markdown을 쓰고 hwpx로 만드는 길이 가장 짧다. 다른 길도 열려 있다. 마켓플레이스에서 엑셀이나 한글 문서를 다루는 다른 스킬을 찾아 설치해도 되고, 4주차처럼 이 절차를 내 스킬로 만들어 두어도 된다. 다만 무엇을 설치하든 1회차 1.3의 네 가지 확인을 먼저 하고, 어떤 도구로 무엇을 했는지 보고서 붙임에 적는다. 도구가 만든 hwpx는 한글이나 한컴독스에서 열어 눈으로 확인한다. 열리지 않거나 표가 깨진 파일은 제출물이 아니다.

**보고서 작성 기준.** 양식의 다섯 절을 모두 채우되 절마다 요구가 다르다. 개요에는 분석 목적 한 문장, 자료의 이름과 출처와 파일 수, 그리고 방법을 적는다. 방법에는 어느 칸에서 값을 뽑았는지, 어떤 파일을 왜 뺐는지, 중복을 어떤 기준으로 정리했는지가 들어가야 한다. 현황에는 양식의 표 두 개를 채우고, 표마다 무엇이 보이는지 한 문장을 붙인다. 첫 표는 법인 수와 평균과 중앙값과 합계이고, 둘째 표는 항목별 상위 5곳이다. 단위는 원 그대로 적으면 자릿수가 열넷을 넘으므로 억 원이나 조 원으로 바꾸고 단위를 표 머리에 밝힌다. 문제점은 자료의 문제와 해석의 문제로 나누어 쓰되, 여러분이 실제로 세어서 확인한 것만 쓴다. "빈칸이 있었다"가 아니라 "총자산이 빈 파일이 몇 개였다"여야 한다. 시사점은 현황과 문제점에서 나온 것만 적고, 자료가 말해 주지 않는 것(예를 들어 법인의 운영이 잘되고 있는지)은 적지 않는다. 붙임에는 계산에 쓴 코드와 법인별 집계표 파일 이름을 적고, 어떤 스킬을 설치해 어디에 썼는지 한 줄을 더한다. 요약 상자에는 대상, 가장 중요한 현황 하나, 가장 중요한 문제점 하나를 세 문장 안으로 쓴다. 과장님은 회의 사이에 이 상자만 읽을 가능성이 크다.

**최종 결과는 이런 모습이다.** 저자가 같은 자료로 같은 과제를 한 결과가 그림 5-5다. 과정과 명령은 보여 주지 않는다. 그것을 찾는 것이 과제이기 때문이다.

![그림 5-5. 과제의 최종 결과 예시: 양식대로 만든 보고서 hwpx의 두 쪽](figures/fig05_gongik_report.png)

그림 5-5를 읽어 보자. 왼쪽이 첫 쪽이다. 제목 아래 음영 상자가 요약이고, 로마 숫자 띠가 양식의 다섯 절이다. 오른쪽 둘째 쪽에 현황의 표 두 개가 채워져 있는데, 첫 표에서 평균과 중앙값의 열이 나란히 놓여 그 차이가 한눈에 보이고, 둘째 표에는 같은 법인이 두 번 나오지 않는다. 문제점 절의 두 항목은 모두 개수가 붙어 있다. 여기서 알 수 있는 것은 이 보고서의 값어치가 숫자의 크기가 아니라 "어떤 기준으로 세었는가"가 적혀 있다는 데 있다는 점이다. 여러분의 숫자는 이 그림과 조금 다를 수 있다. 중복을 정리하는 기준이나 빈 파일을 다루는 방식이 다르면 법인 수부터 달라지기 때문이고, 그 기준을 개요에 적어 두었다면 다른 숫자도 맞는 답이다.

**검증과 채점의 기준.** 제출 전에 셋을 확인한다. 첫째, 보고서의 모든 숫자가 법인별 집계표에서 다시 계산되는가. 표의 법인 수와 집계표의 행 수가 같아야 한다. 둘째, 상위 법인 표의 한 곳을 골라 그 법인의 원본 xls를 열어 보고서의 값과 같은지 본다. 셋째, 문제점 절의 개수가 각각 무엇을 세어 나온 것인지 말할 수 있는가. 채점은 숫자가 저자의 것과 같은지가 아니라 다음 넷을 본다. 어느 칸에서 값을 뽑고 무엇을 뺐는지가 개요에 적혀 있는가, 평균과 중앙값이 함께 있고 차이가 설명되어 있는가, 문제점이 실제로 센 개수로 적혀 있는가, hwpx가 한글에서 열리고 양식의 절과 표가 갖추어져 있는가. 첫 제출본이 이 넷을 채우면 통과이고, 과제 5-C의 뒷부분은 그 보고서의 품질을 한 단계 올리는 일이다.

<div style="border:1px solid #ecd9c6;border-left:5px solid #c77b2f;background:#fdf9f4;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>유의 · 공시 자료 안의 개인정보</strong><br>
이 자료는 국세청이 공개하는 공시 서식이지만, 서식 안에는 대표자 이름과 전화번호, 전자우편주소 같은 개인정보가 들어 있다. 첫 쪽의 대표자 이름은 가려져 있으나 둘째 쪽에는 그대로 적힌 파일이 있다. 과제에 필요한 것은 법인 단위의 재무 항목뿐이므로, 대표자 이름이나 연락처는 집계표에 옮기지 않고 보고서에도 쓰지 않는다. 자료를 다른 곳에 올리지도 않는다. 데이터에서 무엇을 쓰지 않을지 정하는 것도 분석가의 일이며, 14주차에서 다시 다룬다.
</div>

## 2.9 선택 심화: 내가 구한 공문서로 해 보기
...
- [ ] 저장소 주소와 만든 이 계정 이름, MIT 라이선스, 그리고 `.claude-plugin/marketplace.json`과 `plugins/kordoc/skills/kordoc/SKILL.md`의 존재를 브라우저에서 직접 확인했다
- [ ] 이 스킬이 내 컴퓨터에서 npx로 변환 도구를 실행한다는 것을 알고, 설치 확인 화면의 문구를 읽은 뒤 승인했다
- [ ] `/plugin`의 Installed 탭에 kordoc이 있고 Errors 탭이 비어 있으며, 자동완성 목록에 `/kordoc:kordoc`이 뜬다
- [ ] 설치 폴더가 프로젝트 폴더가 아니라 사용자 홈 폴더의 `.claude/plugins/cache/` 아래이고, 경로에 버전 번호가 들어 있다
- [ ] 설치된 SKILL.md의 머리말에 `name`과 `description`이 있고, 4주차에 만든 파일과 뼈대가 같음을 확인했다
- [ ] `data/docs/인구현황보고_예시.md`가 실제로 저장되었고, 작은 제목 네 개와 표 하나, 마지막의 "끝." 표시가 모두 있다
- [ ] `data/인구상위5.csv`가 여섯 줄이고 마지막 데이터 줄이 성남시 922518이며, 총인구가 정수로 읽혀 합계가 5,286,475다
- [ ] 자동 호출 시험에서 스킬이 불렸는지 여부를 도구 사용 로그와 값 대조로 판단했다
- [ ] 문서의 다섯 시군구 총인구를 `data/sigungu_2023.csv`와 대조했고, 그중 하나는 원본 파일에서 직접 확인했다
...
| 과제 자료의 xls 파일을 여는 순간 오류가 난다 | 오래된 엑셀 형식을 읽는 패키지가 환경에 없다 | 오류 문구에 나온 패키지를 환경에 더하게 한다(3주차 2.6). 더한 부품은 설계도에 남는다 |
| 3,103개를 다 집계했다는데 총자산이 빈 파일이 100개 넘게 나온다 | 앞선 판의 서식이 섞여 있어 이름표가 맞지 않는다(2.8 실마리 2) | 서식 판별 기준을 정해 개요에 적고, 앞선 판은 따로 세어 문제점에 적는다 |
| 상위 법인 표에 같은 재단이 두 번 나온다 | 두 사업연도의 공시가 모두 들어 있다(2.8 실마리 4) | 한 법인에 하나만 남기는 기준을 정해 다시 집계하고 그 기준을 개요에 적는다 |
| 만든 hwpx가 한글에서 열리지 않거나 표가 깨진다 | 생성 도구의 산출물 문제이거나 표가 너무 넓다 | 한글이나 한컴독스에서 열어 확인한 뒤 열 수를 줄이거나 다른 프리셋으로 다시 만든다. 열리지 않는 파일은 제출물이 아니다 |
...
**과제 5-B. 내가 구한 공문서 한 건.** 2.9에서 각자 내려받은 문서로 같은 절차를 밟아 제출한다. 제출물은 원본 문서 파일과 그 출처 주소, 변환된 md 파일, 그리고 문서에 표가 있었다면 그 표를 옮긴 CSV 하나다. 여기에 한 문단을 덧붙인다. 이 문서에서 변환이 잘된 부분과 잘 안 된 부분이 무엇이었는지, 그리고 안 된 부분을 앞으로 어떻게 다룰 것인지다. 마지막으로 1회차 1.5절의 판단을 한 문장으로 적는다. 앞으로 이런 일을 만날 때 남이 만든 스킬을 가져다 쓸 것인지, 내 절차를 따로 만들 것인지, 그 판단의 근거는 무엇인지다. 평가 기준은 변환이 잘 되었는지가 아니라(문서마다 다르다) 무엇이 되고 무엇이 안 되었는지를 스스로 확인해 적었는가이다.

**과제 5-C. 공시 서식 3,103장을 보고서 한 장으로.** 2.8의 과제다. 제출물은 셋이다. (1) 양식대로 만든 보고서 hwpx, (2) 법인별 집계표 CSV와 계산에 쓴 코드, (3) 어떤 스킬을 설치해 어디에 썼는지와, 설치 전에 무엇을 확인했는지 적은 한 문단. 첫 제출본은 2.8의 채점 기준 넷을 채우면 통과다. 통과한 뒤에는 같은 보고서의 품질을 한 단계 올려 다시 낸다. 품질을 올리는 방향은 자유롭게 고르되 예를 들면 이렇다. 요약 상자를 과장님이 30초에 읽을 수 있게 다듬는 것, 상위 법인 표에 총자산과 수익을 나란히 두어 순위가 항목마다 다름을 보이는 것, 문제점 절의 개수마다 세는 방법을 붙이는 것, 표의 서식이나 결재란처럼 문서의 꼴을 과의 양식에 더 가깝게 하는 것이다. 두 번째 제출본에는 무엇을 왜 바꾸었는지 한 문단을 붙인다. 평가 기준은 첫 제출본은 채점 기준 넷의 충족 여부이고, 두 번째 제출본은 바꾼 것이 실제로 읽는 사람의 부담을 줄였는지, 그리고 바꾸는 과정에서 숫자가 어긋나지 않았는지다.
