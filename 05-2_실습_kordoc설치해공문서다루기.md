...
진행은 세 단계다. 앞부분(2.1-2.3)은 설치다. 저장소를 먼저 살펴보고, 마켓플레이스를 등록해 플러그인을 설치하고, 설치된 스킬의 SKILL.md를 열어 4주차에 손으로 만든 파일과 같은 구조임을 확인한다. 가운데(2.4-2.5)는 사용이다. 예시 공문서를 Markdown으로 바꾸고, 그 안의 표를 CSV로 옮겨 pandas로 읽는다. 뒷부분(2.6-2.7)은 관찰과 검증이다. 스킬 이름을 부르지 않아도 에이전트가 스스로 꺼내 쓰는지 보고, 변환 결과가 원문과 같은지 사람이 직접 대조한다. 마지막의 2.8은 과제 안내다. 엑셀 서식 3,103장에서 법인별 재무 항목을 모아 과의 양식대로 hwpx 보고서를 만드는 과제를 소개하고, 수업에서는 풀 실마리와 작성 기준만 다룬다.
...
- (과제 5-B) `data/gongik/`의 공시 서식 3,103장을 집계해 양식대로 만든 보고서 hwpx
...
| 데이터 | `data/docs/인구현황보고_예시.hwpx` (수업용으로 만든 가상의 예시 공문서. 실제 행정기관이 발행한 문서가 아니며, 문서 안의 수치는 수업에서 쓰는 공개 데이터에서 가져온 값이다), `data/sigungu_2023.csv` (2.7의 대조에 쓴다. 4주차와 같은 파일), `data/gongik/` (2.8의 과제 자료. 국세청 공익법인 결산서류 공시 표준서식 3,103장, 2020년 공시분, 사회복지 분야. 월별 폴더로 나뉘어 있다. 한 번에 받으려면 `data/gongik.zip`), `data/gongik/보고양식_공익법인분석.hwpx` (과제의 보고 양식. 수업용으로 만든 가상의 과 양식이다) |
...
## 2.8 과제 안내: 엑셀 서식 3,103장을 몇 장의 보고서로

**시나리오.** 이번 과제는 가상의 상황에서 시작한다. 여러분은 어느 구청 사회복지과의 인턴이고, 오늘 아침 과장님이 폴더 하나를 건넸다. 안에는 엑셀 파일 3,103개가 월별 폴더 열두 개에 나뉘어 들어 있다. 파일 하나하나는 공익법인이 국세청에 낸 결산서류 공시 서식(상속세 및 증여세법 시행규칙 별지 제31호서식)이고, 2020년 한 해 동안 공시된 사회복지 분야 법인들의 것이다. 과장님의 말은 짧다. "이거 정리해서 우리 관내 법인 지원 사업 기획할 때 참고하게, 법인들이 대체로 어느 규모인지, 큰 데는 어디인지, 자료에 문제는 없는지 보고서로 만들어 줘. 길 필요 없고 몇 장이면 돼. 양식은 우리 과 것 쓰고." 그리고 과장님은 회의에 들어갔다. 이 과장님과 구청은 수업을 위해 지어낸 것이지만, 폴더 안의 파일은 실제 공시 자료이고 이런 부탁은 실제 행정에서 흔하다.

**과제.** 폴더의 파일에서 법인마다 총자산, 수익, 사업비용을 뽑아 한 표로 모으고, 그것을 집계해 다음을 담은 보고서를 과의 양식에 맞춰 hwpx 파일로 만든다. 법인 수와 세 항목의 평균과 중앙값, 항목별 상위 법인, 그리고 자료를 다루면서 발견한 문제점이다. 재료는 두 가지다. 자료 폴더 `data/gongik/`에는 월별 폴더 열두 개와 3,103개의 xls 파일이 있고, 같은 폴더의 `data/gongik/보고양식_공익법인분석.hwpx`가 과의 보고 양식이다. 자료와 양식을 한 번에 받으려면 `data/gongik.zip`을 내려받아 프로젝트 폴더의 data 안에 풀면 되고, 그러면 `data/gongik/` 폴더가 그대로 생긴다. 양식은 개요, 현황, 문제점, 시사점, 붙임의 다섯 절로 되어 있고 현황 절에 표 두 개가 비어 있다. 이 과제는 수업 시간에 풀지 않는다. 수업에서는 아래의 실마리와 작성 기준을 안내하고, 각자 집에서 에이전트와 함께 완성해 과제 5-B로 낸다.
...
**검증과 채점의 기준.** 제출 전에 셋을 확인한다. 첫째, 보고서의 모든 숫자가 법인별 집계표에서 다시 계산되는가. 표의 법인 수와 집계표의 행 수가 같아야 한다. 둘째, 상위 법인 표의 한 곳을 골라 그 법인의 원본 xls를 열어 보고서의 값과 같은지 본다. 셋째, 문제점 절의 개수가 각각 무엇을 세어 나온 것인지 말할 수 있는가. 채점은 숫자가 저자의 것과 같은지가 아니라 다음 넷을 본다. 어느 칸에서 값을 뽑고 무엇을 뺐는지가 개요에 적혀 있는가, 평균과 중앙값이 함께 있고 차이가 설명되어 있는가, 문제점이 실제로 센 개수로 적혀 있는가, hwpx가 한글에서 열리고 양식의 절과 표가 갖추어져 있는가. 첫 제출본이 이 넷을 채우면 통과이고, 과제 5-B의 뒷부분은 그 보고서의 품질을 한 단계 올리는 일이다.
...
**과제 5-B. 공시 서식 3,103장을 몇 장의 보고서로.** 2.8의 과제다. 제출물은 셋이다. (1) 양식대로 만든 보고서 hwpx, (2) 법인별 집계표 CSV와 계산에 쓴 코드, (3) 어떤 스킬을 설치해 어디에 썼는지와, 설치 전에 무엇을 확인했는지 적은 한 문단. 첫 제출본은 2.8의 채점 기준 넷을 채우면 통과다. 통과한 뒤에는 같은 보고서의 품질을 한 단계 올려 다시 낸다. 품질을 올리는 방향은 자유롭게 고르되 예를 들면 이렇다. 요약 상자를 과장님이 30초에 읽을 수 있게 다듬는 것, 상위 법인 표에 총자산과 수익을 나란히 두어 순위가 항목마다 다름을 보이는 것, 문제점 절의 개수마다 세는 방법을 붙이는 것, 표의 서식이나 결재란처럼 문서의 꼴을 과의 양식에 더 가깝게 하는 것이다. 두 번째 제출본에는 무엇을 왜 바꾸었는지 한 문단을 붙인다. 평가 기준은 첫 제출본은 채점 기준 넷의 충족 여부이고, 두 번째 제출본은 바꾼 것이 실제로 읽는 사람의 부담을 줄였는지, 그리고 바꾸는 과정에서 숫자가 어긋나지 않았는지다.
