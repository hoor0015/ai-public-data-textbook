...
- 세 검색어 비교표 (검색어에 따라 어떤 표가 나오는지 나란히 적어 둔 것)
- 통계표 적합성 판정표 (표 6-4를 채운 것)
- `data/sido_pop_2023.csv`: MCP로 수집한 17개 시도의 2023년 주민등록 총인구
- `data/korea_pop_2019_2023.csv`: 전국 주민등록 총인구의 2019-2023년 추이와 손 검산표
- `수집기록.md`: 언제, 어디서, 어떤 조건으로 받았고 어떻게 검증했는지의 기록 (수집 2건)
- `reports/프로파일_sido_pop_2023.md`: 4주차 스킬로 만든 첫 진단 보고서
- `.claude/skills/kosis-collect/`: 오늘의 수집 절차를 굳힌 스킬 (SKILL.md와 참조 파일)
...
| 도구 | VSCode + Claude Code, 웹 브라우저 (포털 탐색·검증용), 계산기 (2.7의 손 검산용) |
| 선수 내용 | 6주차 1회차 (포털 지도, 메타데이터, MCP), 4주차 2회차 (csv-profile 스킬), 5주차 (스킬의 인자와 스크립트), 2주차 (도구 호출 로그 읽기, 지시문의 검증 손잡이) |
...
읽는 법은 간단하다. 한 묶음은 도구 이름, 도구에 넘긴 인자(검색어나 주소), 돌아온 것의 요약 세 줄이다. 2주차에서 도구 호출 로그 읽는 법을 배운 이유가 여기서 드러난다. 후보 표의 어느 줄이 실제로 페이지를 열어 확인한 것이고 어느 줄이 검색 결과 제목만 보고 적은 것인지는, 표가 아니라 로그를 펼쳐야 갈린다. 세 번째 묶음처럼 페이지를 열지 못한 자리가 있으면 그 후보의 링크는 잠시 뒤 검증에서 가장 먼저 눌러 볼 대상이다.

**검증.** 이 단계의 검증이 오늘 전반부에서 가장 중요하다. 에이전트가 준 링크를 하나도 빠짐없이 브라우저에서 직접 클릭해 열어 본다. 실제로 해 보면 세 가지 경우를 만난다. (1) 링크가 정상이고 설명과 일치한다. (2) 페이지는 열리지만 내용이 설명과 다르다(예: 시군구 단위라고 했는데 실제로는 시도 단위다). (3) 링크가 아예 열리지 않거나 엉뚱한 곳으로 간다. 웹 검색 결과를 조합하는 과정에서 에이전트는 오래된 주소를 가져오거나, 그럴듯한 주소를 지어내기도 한다. 2주차에서 본 "확인할 수 없는 답"이 링크의 형태로 나타나는 것이다. (2)나 (3)을 발견했다면 그 사례를 캡처해 두고, 후보 표 아래에 "후보 1 정상, 후보 2 불일치, 후보 3 불통"처럼 분류 결과를 한 줄로 적는다. 이런 불확실성이야말로, 잠시 뒤 MCP처럼 원천에 직접 연결된 통로가 필요한 이유이기도 하다.
...
터미널에 서버가 추가되었다는 확인 메시지가 나온다. 문구는 버전마다 조금 다르지만 서버 이름과 주소, 등록 범위(따로 지정하지 않으면 이 프로젝트에서 나만 쓰는 범위)가 함께 표시되고, 에이전트가 "새 세션부터 인식된다"는 설명을 덧붙인다. 새로 등록한 서버는 새 세션부터 인식되므로 Claude Code를 재시작한다(4주차에서 스킬 폴더를 새로 만들었을 때와 같다). 그다음 대화창에 `/mcp`를 입력하면 등록된 서버 목록에 kosis가 보이고, 연결 상태와 이 서버가 제공하는 도구들을 확인할 수 있다. 화면 구성은 버전마다 다르지만 대체로 다음과 같은 정보가 보인다.
...
에이전트가 등록된 서버 목록을 확인하는 명령을 먼저 실행해 현재 범위를 보여 주고, 이어서 범위를 사용자로 지정하는 옵션을 붙여 같은 등록 명령을 다시 실행한다. 화면에는 서버 이름과 주소, 그리고 어느 범위에 저장되었는지가 함께 표시된다. 문구는 버전마다 다르지만 읽을 것은 셋이다. 이름이 kosis인가, 주소가 내가 준 주소인가, 범위가 사용자인가. 재시작한 뒤 다른 폴더를 열어 `/mcp`를 눌러 보면 kosis가 그대로 보인다. 범위를 옮겼다는 사실이 화면으로 확인되는 순간이다.

**일부러 틀린 주소로 등록해 보기.** 이제 짧은 실험을 하나 한다. 등록이 성공했다는 메시지와 서버가 실제로 살아 있다는 사실은 다른 이야기라는 것을 확인하는 실험이다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"실험을 하나 하려고 해. 일부러 틀린 주소로 MCP 서버를 하나 더 등록해 줘. 이름은 kosis-test, 주소는 https://kosismcp2026.vercel.app/api/mcp-test 야. 등록한 뒤 서버 목록과 연결 상태를 확인하고, 화면에 나온 메시지를 요약하지 말고 그대로 보여 줘. 확인이 끝나면 kosis-test 서버를 지워 줘."
</div>

**예상되는 실험 결과.** 등록 자체는 성공한다. 등록 명령은 주소를 받아 적을 뿐 그 주소에 서버가 있는지 확인하지 않기 때문이다. 그래서 화면에는 kosis 때와 거의 같은 성공 메시지가 나온다. 차이는 연결 상태를 확인할 때 드러난다. 서버 목록이나 `/mcp` 화면에서 kosis-test는 연결됨이 아니라 실패로 표시되고, 그 옆이나 아래에 이유가 붙는다.

```text
MCP 서버
  kosis        연결됨    http    https://kosismcp2026.vercel.app/api/mcp
  kosis-test   실패      http    https://kosismcp2026.vercel.app/api/mcp-test
                         (연결하지 못했다는 메시지가 이어진다)
```

여기서 얻을 것이 둘이다. 첫째, 등록 성공은 "주소를 적어 두었다"는 뜻이고 연결됨은 "그 주소에 실제로 서버가 있고 대화가 된다"는 뜻이다. 두 문장은 다르며, 그래서 등록 다음에 반드시 연결 확인이 온다. 둘째, 오류 메시지를 읽는 연습이다. 메시지에 주소가 통째로 다시 적혀 있으므로, 실패했을 때 가장 먼저 할 일은 그 주소를 내가 준 주소와 한 글자씩 대조하는 것이다. 연결이 되지 않을 때 가장 먼저 의심할 것은 서버가 아니라 주소의 오타다. 확인이 끝나면 에이전트가 실험용 서버를 지우고, 목록에 kosis만 남는다. 쓰지 않는 서버를 남겨 두지 않는 것도 1회차에서 배운 점검 항목이었다.

**검증.** ① 에이전트가 실행한 명령을 눈으로 읽는다. 이름(kosis)과 주소가 지시한 그대로인지 확인한다. ② 재시작 후 `/mcp` 화면에서 kosis 서버의 상태가 연결됨으로 표시되는지 확인한다. ③ 도구 목록에 검색, 구조 확인, 조회에 해당하는 도구가 보이는지 훑어본다. ④ 등록 범위가 사용자로 바뀌었는지 확인한다. 가장 확실한 확인은 다른 폴더를 하나 열어 `/mcp`를 눌러 보는 것이다. 거기서도 kosis가 보이면 범위가 옮겨진 것이다. ⑤ 실험용으로 만든 kosis-test가 목록에서 사라졌는지 확인한다. 남아 있으면 다음 시간에 실패 표시가 계속 보여 진짜 문제를 가린다. ⑥ 마지막으로 등록 명령의 의미를 자기 말로 설명할 수 있는지 점검한다. `kosis`는 내가 붙인 별명이고, 주소는 서버의 위치이며, 이 서버는 읽기 전용 통계 도구를 제공한다. 무엇을 꽂았는지 모른 채 꽂는 것이 1회차에서 경고한 가장 나쁜 습관이다.
...
인자의 이름은 서버 버전에 따라 조금씩 다르게 표시되지만, 무엇을 넘겼는지는 이렇게 두세 줄로 읽을 수 있다. 검색 도구에는 말(검색어)을 넘기고 구조 확인 도구에는 ID를 넘긴다는 차이를 눈여겨보자. 말은 흔들리고 ID는 흔들리지 않는다는 것이 다음 절의 주제다. 저자가 실제로 수행했을 때 에이전트는 "행정구역(시군구)별 성별 인구수"(통계표 ID DT_1B040A3)를 골랐다. 주민등록인구현황 통계에 속한 표로, 원자료는 행정안전부의 주민등록 자료이고 KOSIS(국가데이터처(옛 통계청) 운영)에 수록되어 있다. 구조 확인 결과는 대략 다음 모양으로 정리되어 온다. 여러분 화면에서 확인해야 하는 값은 ○로 비워 두었다.
...
여기서 읽을 것이 셋 있다. 첫째, 항목이다. 총인구수 말고 성별 항목도 있으므로, 조회할 때 총인구수를 지정하지 않으면 남자·여자 값이 섞여 올 수 있다. 둘째, 분류의 계층이다. 표 이름에 "시군구"가 들어 있듯 이 표는 전국, 시도, 시군구를 한 계층 구조로 담고 있고, 우리는 그중 시도 수준만 원한다. 다음 단계에서 이 계층이 실제로 말썽을 일으킨다. 셋째, 기준이다. 주민등록 기준 인구는 인구총조사 기준 인구와 다르다(1회차 1.2절의 첫 확인 항목). 읽은 내용을 표 6-4로 옮긴다. 1회차의 다섯 확인 항목에 이용허락을 더한 것이다.
...
| 기준 시점과 기간 | 2023년, 가능하면 여러 해 | 연도별 수록. 2019-2023년 다섯 해 조회 가능 (2.7에서 확인) | 맞음 |
| 단위와 산식 | 명 단위의 총인구 | 단위 명, 항목 총인구수 (남자+여자) | 맞음 |
| 범위와 분류 | 17개 시도 전체 | 전국 > 시도 > 시군구 계층. 시도 수준만 지정해야 함 | 맞음 (조건부) |
| 갱신 주기와 이력 | 나중에 갱신할 가능성 | KOSIS 화면의 수록 정보에서 확인해 적는다 | (화면 확인) |
| 이용허락과 출처 표시 | 보고서 인용 가능 | KOSIS 화면에서 공공누리 유형을 확인해 적는다 | (화면 확인) |

**검증.** 판정표의 근거를 포털 화면에서 확인한다. 브라우저에서 KOSIS에 접속해 통계표 "행정구역(시군구)별 성별 인구수"를 검색해 열고, 항목 선택 창에 총인구수·남자인구수·여자인구수가 있는지, 지역 선택 창이 전국·시도·시군구의 계층인지, 시점 선택 창에서 몇 년부터 고를 수 있는지 눈으로 본다. MCP가 알려 준 구조와 화면이 같아야 한다. 확인이 끝나면 표 6-4의 빈 칸(갱신 주기, 이용허락)을 화면에서 읽은 대로 채운다. 한 가지 더. 검색어에 "인구"만 넣으면 에이전트가 인구총조사 계열의 표를 고를 수도 있다. 표 이름에 "주민등록"이 없거나 판정표의 첫 행이 어긋나면 "주민등록 기준의 표로 다시 골라 줘"라고 재지시한다. 표를 잘못 고르면 이후의 모든 숫자가 정확하면서도 틀린 답이 된다.

## 2.4 검색어를 바꾸면 다른 표가 나온다: 세 검색어로 같은 통계 찾기

**무엇을 왜 하는가.** 2.3에서는 "주민등록 인구"라는 검색어 하나로 표를 찾았고 다행히 원하던 표에 닿았다. 그러나 검색은 창구 직원에게 말을 거는 일과 비슷해서, 어떤 말로 묻느냐에 따라 열리는 서랍이 달라진다. 같은 통계를 세 가지 말로 찾아 결과를 나란히 놓아 보면 두 가지를 얻는다. 첫째, 검색어가 결과를 정한다는 사실을 눈으로 본다. 둘째, 내가 쓰는 말 가운데 어느 것이 원하는 표에 닿는지 알게 된다. 이 감각은 오늘 한 번 쓰고 마는 것이 아니다. 7주차에서 병합할 두 번째 데이터를 찾을 때도, 12주차에서 근거로 쓸 통계를 찾을 때도, 학기 프로젝트에서 새 지표가 필요할 때마다 쓴다. 값은 아직 받지 않는다. 2.3과 마찬가지로 검색만 하는 단계다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"kosis MCP의 검색 도구로 다음 세 검색어를 각각 따로 검색해 줘. (가) 인구 (나) 주민등록 인구 (다) 행정구역별 인구. 검색어마다 상위 결과 3개의 통계표 이름과 ID를 적고, 세 검색어의 결과를 한 표에 나란히 놓아 줘. 그리고 (1) 2.3에서 고른 DT_1B040A3이 어느 검색어에서 나왔는지 표시하고, (2) 검색어에 따라 결과가 달라진 이유에 대한 너의 설명을 한두 문장으로 덧붙여 줘. 값은 조회하지 말고 검색만 해. 검색 도구를 몇 번 호출했는지도 알려 줘."
</div>

**예상되는 에이전트의 행동과 결과.** 도구 사용 로그에 검색 도구 호출이 세 번 나타난다. 한 번만 호출하고 나머지 두 검색어의 결과는 첫 결과에서 추려 내는 경우가 있으므로, 호출 횟수를 보고하라는 요구를 지시문에 넣어 두었다. 로그의 모양은 다음과 같다.

```text
[도구 호출] kosis_search   keyword: "인구"
[결과] 통계표 후보 목록 (표 이름과 ID가 한 줄에 하나씩)
[도구 호출] kosis_search   keyword: "주민등록 인구"
[결과] 통계표 후보 목록
[도구 호출] kosis_search   keyword: "행정구역별 인구"
[결과] 통계표 후보 목록
```

이어서 비교표가 온다. 검색 결과는 서버가 개편되면 달라지므로 아래 표에서 볼 것은 칸의 내용이 아니라 표의 꼴이다. 각 칸은 여러분 화면에 나온 표 이름과 ID로 채운다.

| 순위 | (가) "인구" | (나) "주민등록 인구" | (다) "행정구역별 인구" |
|------|------|------|------|
| 1 | (화면에서 확인한 표 이름과 ID를 적는다) | (화면에서 확인한 표 이름과 ID를 적는다) | (화면에서 확인한 표 이름과 ID를 적는다) |
| 2 | | | |
| 3 | | | |
| DT_1B040A3이 있는가 | | | |

표를 다 채웠으면 세 열을 나란히 읽어 본다. 대체로 넓은 말일수록 여러 계열의 표가 섞여 나오고, 기준을 밝힌 말일수록 후보가 좁아진다. "인구"처럼 넓은 말에는 주민등록 계열뿐 아니라 인구총조사 계열이나 장래인구추계 계열의 표가 함께 걸릴 수 있고, 2.3의 검증에서 미리 경고한 "표 이름에 주민등록이 없는 표"가 여기서 실제로 목록에 나타나기도 한다. 반면 "주민등록"처럼 기준을 못 박은 말은 후보를 줄여 준다. 검색어에 조사 이름이나 기준을 한 마디 넣는 것이 검색 범위를 좁히는 가장 싼 방법이다.
...
<strong>유의 · 검색어보다 통계표 ID</strong><br>
검색어는 사람이 기억하기 쉬운 대신 결과가 흔들린다. 통계표 ID(DT_1B040A3)는 외우기 어려운 대신 언제나 같은 표를 가리킨다. 그래서 한 번 찾은 표는 이름이 아니라 ID로 부르고, 수집기록에도 ID를 반드시 남긴다. 2.6의 수집기록 양식에 통계표 이름과 ID를 함께 적는 칸이 있는 이유이며, 2.9에서 만들 스킬이 검색어가 아니라 ID를 인자로 받는 이유이기도 하다. 검색은 처음 한 번이고, 그다음부터는 ID다.
</div>

**검증.** ① 도구 사용 로그를 펼쳐 검색 호출이 정말 세 번 있었는지 센다. 두 번뿐이라면 어느 한 검색어의 결과는 조회가 아니라 짐작이므로, 그 열은 비워 두고 다시 시킨다. ② 비교표의 마지막 행을 채운다. DT_1B040A3이 세 검색어 모두에서 나왔는가, 한두 곳에서만 나왔는가. 나오지 않은 검색어가 있다면 그것이 오늘 배울 것의 핵심이다. 같은 표가 말에 따라 보이기도 하고 보이지 않기도 한다. ③ 검색어 하나를 골라 KOSIS 화면의 검색창에도 같은 말을 넣어 보고, 화면의 결과 목록과 MCP의 결과가 얼마나 겹치는지 훑는다. 두 문이 같은 창고를 향하고 있으므로 크게 어긋나서는 안 되고, 크게 어긋난다면 검색 도구가 무엇을 훑고 있는지 다시 물어볼 일이다. ④ 마지막으로 한 줄 메모를 남긴다. "내가 찾던 표에 닿은 검색어는 (나)였고 (가)는 후보가 너무 넓었다"처럼 적어 두면, 다음에 새 통계를 찾을 때 그 말버릇부터 꺼내 쓸 수 있다.

## 2.5 MCP로 통계 수집하기: 2023년 시도별 총인구
...
## 2.6 수집 결과 검증과 저장: 화면 대조, CSV, 수집기록
...
1. KOSIS(kosis.kr)에 접속해 검색창에 통계표 이름 "행정구역(시군구)별 성별 인구수"를 그대로 넣고 검색한다. 결과 목록에서 이름이 정확히 같은 통계표를 골라 연다. 이름이 비슷한 표가 여럿이면 2.3에서 확인한 통계표 ID(DT_1B040A3)가 함께 표시되는 쪽을 고른다.
2. 표가 열리면 화면 위쪽의 조회 설정 영역을 본다. 항목, 행정구역(분류), 시점이 각각 버튼이나 탭으로 나뉘어 있다. 이 셋이 바로 MCP 조회에서 도구에 넘긴 조건에 해당하는 자리다. 화면의 이 영역과 도구 호출의 조건이 하나씩 짝을 이룬다는 사실을 눈으로 확인해 두면, 앞으로 조회 조건을 지시할 때 무엇을 적어야 하는지가 분명해진다.
3. 시점에서 2023년 하나만 남긴다. 여러 해가 선택되어 있으면 값이 여러 열로 펼쳐져 대조가 번거롭다.
4. 항목에서 총인구수만 남기고(남자인구수와 여자인구수는 해제), 행정구역에서 전국, 서울특별시, 세종특별자치시 셋만 선택한 뒤 조회를 누른다. 여기서 시군구를 함께 선택하지 않는 것이, 앞서 2.5의 검증에서 겪은 계층 문제를 화면 쪽에서 다시 만나는 자리다.
5. 화면에 나온 세 값을 표 6-5에 옮겨 적고 에이전트가 받은 값과 자릿수까지 비교한다. 이때 표 위아래에 적힌 단위 표기와 수록 시점 표기도 함께 적어 둔다. 값만 맞추고 단위를 흘려 읽으면 뒤에서 숫자가 천 배로 어긋난다.

**표 6-5. 화면 대조 기록표 (세 값, 2023년)**

| 지역 | MCP로 받은 값 (명) | KOSIS 화면의 값 | 일치 여부 | 화면에서 함께 확인한 것 |
|------|------|------|------|------|
| 전국 | 51,325,329 | (화면에서 확인한 값을 적는다) | | 단위 표기 |
| 서울특별시 | 9,386,034 | (화면에서 확인한 값을 적는다) | | 시점 표기 |
| 세종특별자치시 | 386,525 | (화면에서 확인한 값을 적는다) | | 통계표 이름과 ID |
...
**표 6-6. data/sido_pop_2023.csv의 첫 다섯 행 (실제 수집 결과)**
...
재지시 뒤에는 열 이름이 "총인구"로 돌아오고, 요약 문장이 "전국 51,325,329명(약 5,133만 명)"처럼 바뀐다. 표 6-6의 열 이름이 단위 없이 그냥 "총인구"인 이유가 여기에 있다. 파일에는 원래 값을 그대로 두고, 줄여 읽는 일은 사람이 보는 화면과 문장에서만 한다. 7주차에서 이 파일을 다시 열 때, 열 이름 하나가 잘못 붙어 있으면 그때는 되돌릴 근거조차 남아 있지 않다.
...
검증 내용 칸을 직접 채우라고 한 데는 이유가 있다. 무엇을 어떻게 대조했고 결과가 어땠는지는 검증한 사람만이 쓸 수 있는 문장이다. "KOSIS 화면에서 서울 9,386,034 일치 확인, 시도 합계 51,325,329 = 전국 값 일치"처럼 구체적으로 쓴다. 이 한 줄이 14주차 최종 보고서에서 "이 숫자의 근거는 무엇인가"라는 질문에 대한 답이 된다. 완성된 수집기록은 표 6-7과 같은 모양이어야 한다. 오른쪽 칸은 오늘 첫 번째 수집을 저자의 결과로 채운 예다.

**표 6-7. 수집기록 양식과 기입 예 (수집 1: 2023년 시도별 총인구)**
...
| 저장 파일 | data/sido_pop_2023.csv (17행 3열, utf-8-sig), figures의 막대그래프 |
| 재수집 지시문 | 2.5의 지시문(재지시 손잡이 포함)과 2.6의 저장 지시문 전문 |

**검증.** ① VSCode에서 data/sido_pop_2023.csv를 열어 헤더 1행 + 데이터 17행인지, 한글이 멀쩡한지 확인한다. 마지막 행 번호가 18이면 맞다. ② 표 6-6의 첫 행(경기도, 13630821)이 파일의 2번째 줄과 같은지, 총인구 값에 쉼표나 따옴표가 섞여 문자열이 되지 않았는지 본다. ③ 수집기록.md의 모든 칸이 채워졌는지, 특히 검증 내용이 자기 문장으로 적혔는지 확인한다. ④ 최종 확인으로 새 대화를 열고, 수집기록에 적어 둔 지시문으로 처음부터 다시 수집해 본다. 같은 18건, 같은 값이 나오면 여러분은 재현 가능한 수집 절차를 갖게 된 것이다. 이것이 2주차에서 배운 새 대화 독립 검산이며, 13주차에서는 이 일을 검증 전담 서브에이전트(verifier)에게 맡긴다. 그때 서브에이전트에게 건네는 지시서의 재료가 오늘의 수집기록이다.

## 2.7 두 번째 수집: 전국 총인구 2019-2023년 추이와 손 검산
...
지시문이 2.5보다 길어진 것은 성공 기준(다섯 건), 이상 상황 보고, 계산 금지(증감은 내가 한다), 안내문 원문 요구가 더해졌기 때문이다. 모두 2.5와 2.6에서 겪은 일에서 나온 손잡이다.
...
표 아래에 "5건입니다"라는 건수 보고와 출처, 첫머리에 시범서비스 안내가 붙는다. 이 단계에서 만나는 틀리는 장면은 둘인데, 둘 다 값이 아니라 형식에서 생긴다.
...
**틀리는 장면 2. 안내문이 사라진다.** 값은 제대로 왔는데 첫머리의 시범서비스 안내가 없는 경우가 있다. 에이전트가 답을 정리하면서 서버의 안내를 요약하거나 생략한 것이다. 값에는 문제가 없으므로 넘어가기 쉽지만, 안내문의 출처 링크와 한계 고지는 수집기록의 재료다. 2.5의 응답과 비교해 알아챘다면 "서버 응답에 안내문이 있었는지 확인하고, 있었다면 요약하지 말고 원문 그대로 붙여 줘"라고 재지시한다. 응답이 올 때마다 안내문의 유무를 먼저 보는 버릇을 들이자.
...
**검증.** 이번 검증의 핵심은 손 검산이다. 에이전트에게 증감을 계산하지 말라고 한 이유가 여기에 있다. 여러분이 먼저 계산하고 에이전트의 계산은 나중에 받아 대조한다. 순서를 바꾸면 에이전트의 숫자를 보고 "맞는 것 같다"고 넘어가게 되므로 반드시 손이 먼저다. 계산기로 표 6-8의 빈 칸을 채운다.

**표 6-8. 손 검산표 (왼쪽 두 열은 수집 결과, 오른쪽 두 열은 직접 계산)**
...
<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"방금 받은 다섯 해의 값을 연도·총인구 두 열의 표로 data/korea_pop_2019_2023.csv 에 저장해 줘(총인구는 숫자, 인코딩 utf-8-sig). 저장한 뒤 파일을 다시 읽어 전년 대비 증감 열을 계산해 화면에 보여 주되, 파일에는 더하지 마. 그리고 연도별 총인구 선그래프와 전년 대비 증감 막대그래프를 나란히 그려 figures 폴더에 저장해 줘. 축에 단위(만 명)를 표시하고, 증감 그래프에는 0을 기준선으로 그려 줘."
</div>

에이전트가 보여 주는 증감 열을 표 6-8의 여러분 계산과 한 칸씩 대조한다. 네 칸이 모두 같으면 사람과 에이전트가 독립적으로 같은 결과에 이른 것이고, 다르면 계산식을 다시 따라가 어느 쪽이 틀렸는지 가린다. 파일에 증감 열을 넣지 말라고 한 것은 원본은 그대로 두고 가공은 사본으로 한다는 1회차 1.2절의 습관 때문이다.

![그림 6-4. 전국 주민등록 총인구 추이(2019-2023)와 전년 대비 증감, 두 번째 수집 결과의 확인 그림](figures/fig06_korea_trend.png)

그림 6-4를 읽어 보자. 왼쪽 (가)는 다섯 해의 총인구를 선으로 이은 것이고 점 위의 숫자가 수집한 값 그대로다. 오른쪽 (나)는 표 6-8의 증감 열을 막대로 그린 것으로, 네 막대가 모두 0의 기준선 아래에 있다. 여기서 알 수 있는 것은 감소가 한 해의 우연이 아니라 네 해 연속이라는 점, 그리고 2021-2022년의 감소 폭이 2020년의 열 배 가까이 크다는 점이다. 주의할 것이 하나 있다. (가)의 세로축은 0이 아니라 5,100만 명부터 시작하므로 선이 가파르게 보이지만 실제 감소율은 5년간 약 1%다. 확인용 그림에서는 축을 좁혀야 변화가 보이지만, 보고서에 실을 때는 9주차에서 배울 축 절단의 규칙을 따른다.

마지막으로 "수집기록.md에 두 번째 수집을 같은 양식으로 추가해 줘. 검증 내용 칸은 비워 둬"라고 지시하고, 검증 내용에는 "KOSIS 화면에서 2019년 51,849,861 일치 확인, 손 검산 증감 4건이 에이전트 계산과 일치, 재지시 1회(기간 누락)"처럼 실제로 한 일을 적는다. 재지시가 있었다면 그것도 기록이다. 어디서 틀렸는지 적어 둔 기록이 다음 수집의 지시문을 고쳐 준다. 완성된 두 번째 항목은 표 6-9과 같은 모양이 된다.

**표 6-9. 수집기록 기입 예 (수집 2: 전국 총인구 2019-2023년 추이)**

| 항목 | 기록 |
|------|------|
| 수집일 | 수집 1과 같은 날 (예: 2026-08-12, 저자의 수집일) |
| 수집 경로 | kosis MCP (서버 이름 kosis, https://kosismcp2026.vercel.app/api/mcp) |
| 원자료와 작성 기관 | 행정안전부 주민등록인구현황. KOSIS(국가데이터처) 수록 |
| 통계표 | 행정구역(시군구)별 성별 인구수 (DT_1B040A3). 수집 1과 같은 표 |
| 조회 조건 | 항목 총인구수, 지역 전국 1건, 시점 2019년부터 2023년까지 연도별 |
| 도구 호출 | 조회 2회 (첫 조회에서 기간이 잘려 재지시. 로그에서 센 대로 적는다) |
| 수신 건수와 구성 | 5건 = 전국 1개 지역 × 5개 연도 |
| 서버 안내문 | 시범서비스 안내를 원문 그대로 확인. 출처 링크: (안내문의 링크를 그대로 옮긴다) |
| 검증 내용 (내가 씀) | KOSIS 화면 대조: 2019년 51,849,861 일치, 2023년 51,325,329 일치. 손 검산 증감 4건이 에이전트 계산과 일치. 증감 합계 -524,532 = 누적 증감 일치. 기준 시점 설명은 화면에서 확인해 보완 |
| 저장 파일 | data/korea_pop_2019_2023.csv (5행 2열, utf-8-sig), figures의 선그래프와 증감 막대그래프 |
| 재수집 지시문 | 2.7의 조회 지시문(기간 명시, 건수 보고, 계산 금지 포함)과 저장 지시문 전문 |
| 이 수집을 쓰는 곳 | 12주차 근거 메모("우리나라 인구는 감소 추세다"의 근거) |

두 기록을 나란히 놓고 보면 달라진 칸은 다섯이다. 조회 조건, 도구 호출, 수신 건수와 구성, 검증 내용, 저장 파일이다. 나머지 칸은 글자 하나 바뀌지 않았다. 같은 양식이 두 번 다 들어맞았다는 것은 이 양식이 이 데이터에만 맞는 것이 아니라는 뜻이고, 그렇다면 다음에 할 일은 정해져 있다. 양식과 절차를 손으로 다시 옮겨 적는 대신 스킬로 굳히는 것이다(2.9). 표 6-9의 마지막 행처럼 "이 수집을 어디에 쓰는가"를 한 줄 적어 두면, 몇 주 뒤에 파일 이름만 보고 무엇이었는지 되짚는 수고도 덜 수 있다.

## 2.8 수집한 데이터의 첫 진단: csv-profile 스킬 실행

**무엇을 왜 하는가.** 4주차 실습 끝에 6주차에 수집한 데이터의 첫 진단을 `/csv-profile`로 한다는 예고가 있었다. 그 약속을 지킬 차례다. 수집 직후의 진단은 파일이 내가 생각한 모양(17행 3열, 총인구는 숫자)으로 저장되었는지, 그리고 4주차의 스킬이 처음 보는 파일에서도 제대로 도는지를 확인해 준다. 이 단계가 4주차(절차의 표준화)와 6주차(수집)를 한 흐름으로 잇고 7주차(정제를 위한 진단)로 넘긴다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"/csv-profile data/sido_pop_2023.csv"
</div>

**예상되는 에이전트의 행동과 결과.** 에이전트가 SKILL.md의 절차를 따라 파일 존재를 확인하고, 코드로 계산하고, `reports/프로파일_sido_pop_2023.md`와 `code/profile_sido_pop_2023.py`를 저장했다고 보고한다. 보고서는 4주차에서 정한 다섯 절의 형식을 따르며, 대략 다음 내용이 담긴다.
...
**검증.** 스킬의 산출물도 검증 대상이다. 4주차의 유의 박스가 경고했듯, 형식이 말끔할수록 안을 들여다보는 습관이 무뎌진다. ① 행 수 17이 2.6에서 확인한 파일의 행 수와 같은가. ② 최댓값 13,630,821이 표 6-6의 첫 행(경기도)과 같은가. ③ 평균을 손으로 검산한다. 17개 시도의 합은 2.6에서 전국 값과 같음을 확인했으므로 평균은 51,325,329 ÷ 17이고, 계산기로 나누면 3,019,137로 나누어떨어진다. 중앙값은 17개 값 중 큰 순서로 9번째이므로, 그림 6-3의 막대를 위에서 아홉 번째까지 세면 전라남도(1,804,217)가 나와야 한다. 경고에 대한 판단은 보고서의 "5. 확인 필요" 절 아래에 자기 문장으로 적는다. "연도 열의 단일값 경고는 2023년만 수집한 설계의 결과이므로 문제 아님"이 한 예다.

여기서 5주차를 떠올려 보자. 오늘 "표를 고르고, 조건을 정하고, 조회하고, 건수를 세고, 저장하고, 수집기록을 쓰고, 프로파일링한다"는 순서가 두 번 반복되었다. 4주차의 기준으로 세 번째 반복 직전이다. 5주차에서 배운 대로 통계표 ID와 조회 조건을 인자로 받고 수집기록 양식을 참조 파일로 두면 수집 절차도 스킬이 된다. 다음 절에서 실제로 만들어 본다.

## 2.9 수집 절차를 스킬로 굳히기: kosis-collect

**무엇을 왜 하는가.** 오늘 같은 순서를 두 번 밟았다. 표를 고르고, 조회 조건을 정하고, 몇 건이 와야 하는지 기준을 세우고, 조회하고, 건수를 확인하고, 저장하고, 수집기록을 쓰는 일곱 걸음이다. 두 수집에서 달랐던 것은 조건 두 가지, 곧 지역 수준과 기간뿐이고 나머지는 그대로였다. 4주차에서 배운 판단 기준, 곧 같은 지시를 세 번째 쓰기 전에 절차로 내린다는 기준에 정확히 걸리는 자리다. 여기에 5주차에서 배운 도구를 붙인다. 바뀌는 것은 인자로 빼고, 바뀌지 않는 것은 절차에 고정하며, 매번 참조하는 양식은 옆의 참조 파일에 둔다. 학기 프로젝트에서 새 통계를 받을 때마다 오늘 지시문을 다시 찾아 헤매는 대신 한 줄로 부르는 것이 목표다.

무엇을 인자로 뺄지부터 정한다. 통계표 ID는 수집마다 바뀌므로 인자다. 기간도 바뀌므로 인자다. 지역 수준(전국인지 시도인지 시군구인지)도 바뀌지만 대부분 시도 수준이므로, 인자로 두되 비워 두면 시도로 간주하게 한다. 반면 "조회 전에 건수 기준을 세운다", "받은 건수를 세어 기준과 비교한다", "수치는 MCP 조회 결과만 쓴다", "증감은 계산하지 않는다"는 어느 수집에서나 지켜야 하므로 절차와 규칙에 고정한다. 인자와 고정을 가르는 기준은 하나다. 데이터가 바뀌면 달라지는가, 아니면 언제나 같은가.

![그림 6-5. kosis-collect 스킬의 절차와 검증 손잡이가 걸리는 자리](figures/fig06_skill.png)

그림 6-5를 읽어 보자. 맨 위의 초록 상자가 스킬을 부르는 한 줄이고, 그 아래 파란 상자 여섯 개가 SKILL.md에 적어 둘 절차다. 오른쪽의 주황 쪽지 셋은 절차 안에 박아 넣을 검증 손잡이가 어느 단계에 붙는지 가리킨다. 여기서 알 수 있는 것은 스킬이 일을 줄이는 장치가 아니라 판단을 고정하는 장치라는 점이다. 한 줄로 부르는 대신, 조회 전에 기준을 세우고 조회 후에 세어 보는 두 걸음은 사라지지 않고 절차 안으로 들어간다. 맨 아래 회색 쪽지가 인자와 고정을 가르는 기준을 다시 적어 둔 것이다.
...
SKILL.md에 넣을 내용은 다음과 같다. 5주차에서 익힌 대로 머리말에 인자 이름을 선언하고 본문에서 그 이름으로 받는다.
...
argument-hint: [통계표 ID] [기간, 예 2019-2023] [지역 수준, 예 시도]
arguments: [table, period, level]
---

수집 대상 통계표: $table
조회 기간: $period
지역 수준: $level (비어 있으면 "시도"로 간주한다)

## 절차
1. 구조 확인: kosis MCP의 통계표 구조 확인 도구로 $table 의 항목, 분류, 수록 기간,
   단위를 먼저 읽는다. 통계표를 찾지 못하거나 $period 가 수록 기간을 벗어나면
   조회하지 말고 멈춘 뒤 사용자에게 알린다.
2. 건수 기준 세우기: 조회 전에 "몇 건이 와야 하는가"를 계산해 화면에 적는다.
   (지역 수 곱하기 시점 수. 전국 1개 지역에 5개 연도이면 5건)
3. 조회: 항목은 하나로 지정하고, 지역은 $level 수준으로 한정하며,
   시작 시점과 끝 시점을 $period 에서 읽어 도구에 명시한다.
...
읽어 둘 곳이 셋 있다. 첫째, 본문 셋째 줄의 괄호다. 지역 수준을 빼고 불렀을 때 되묻지 말고 시도로 진행하라는 기본값 규칙이며, 5주차에서 인자를 빠뜨린 실행을 관찰하며 배운 장치다. 둘째, 절차 2와 4가 짝을 이룬다는 점이다. 조회 전에 기준을 적고 조회 뒤에 세어 비교하게 했으므로, 2주차의 성공 기준과 이상 상황 보고가 그때그때의 지시문이 아니라 절차 안에 박혔다. 셋째, 절차 6의 마지막 문장이다. 검증 내용 칸을 비워 두라고 스킬이 스스로 못 박아 두었다. 검증은 사람의 일이라는 오늘의 원칙을 스킬이 대신 기억해 주는 셈이다.

**예상되는 에이전트의 행동과 결과.** 폴더 하나와 파일 두 개가 생겼다는 보고가 온다. 대화창에 `/`를 치면 자동완성 목록에 `/kosis-collect`가 보이고 옆에 인자 힌트가 뜬다. 새 대화를 열어 다음과 같이 부른다.
...
에이전트가 SKILL.md의 절차를 순서대로 밟는 모습이 도구 사용 로그에 나타난다. 아래는 그 로그의 모양이다. 표시 방식은 버전에 따라 다르지만 읽을 것은 같다.

```text
[스킬] kosis-collect   $table=DT_1B040A3  $period=2019-2023  $level=전국
...
한 줄이 2.7에서 여러 번 주고받은 지시를 대신한다. 다만 절차가 짧아 보이는 것과 검증이 줄어드는 것은 다른 이야기다. 파일이 저장되었다는 보고는 아직 검증이 아니다.

**검증.** 세 가지로 확인한다.

① 산출물 대조. 스킬이 저장한 CSV를 열어 다섯 값이 2.7에서 손으로 검산해 둔 값과 한 자리도 다르지 않은지 본다. 이번에는 정답지를 미리 만들어 둔 셈이므로 대조가 빠르다. 스킬이 새로 쓴 수집기록 항목도 앞의 두 기록과 같은 항목이 모두 있는지 훑고, 검증 내용 칸이 비어 있는지 확인한다. 채워져 있다면 스킬이 절차 6의 마지막 문장을 어긴 것이므로 그 문장을 더 분명하게 고친다.

② 오류 시험. 5주차에서 배운 오류 심기를 스킬에도 해 본다. 있지도 않은 통계표 ID로 불러 보는 것이다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"/kosis-collect DT_0000000 2019-2023 전국"
</div>

바람직한 동작은 절차 1의 구조 확인에서 실패하고 멈추어 "이 통계표를 찾을 수 없다"고 보고하는 것이다. 나쁜 동작은 조회를 건너뛰고 기억으로 표를 지어내거나, 비슷한 이름의 다른 표로 슬쩍 바꾸어 진행하는 것이다. 나쁜 동작이 나왔다면 규칙에 "통계표를 찾지 못하면 다른 표로 바꾸지 말고 멈춘다"를 한 줄 더한다. 수록 기간을 한참 벗어난 기간으로 불러 절차 1의 기간 점검이 작동하는지도 같은 방식으로 시험할 수 있다. 스킬을 만든 날 바로 부수어 보는 것이 5주차의 방식이었다.

③ 기준선 비교. 4주차에서 배운 방식대로, 새 대화에서 스킬 없이 "DT_1B040A3에서 전국 총인구를 2019년부터 2023년까지 받아 줘"라고만 시킨 결과와 나란히 놓는다. 볼 것은 세 가지다. 건수 기준을 먼저 세웠는가, 수집기록을 남겼는가, 시키지 않은 증감을 계산해 붙였는가. 세 가지가 갈린다면 오늘 스킬로 굳힌 것이 문장이 아니라 판단이라는 뜻이다.
...
**검증.** 에이전트가 "존재한다"고 한 핵심 데이터는 직접 확인한다. MCP 검색으로 찾은 통계표는 KOSIS에서 열어 분석 단위(시군구인가 시도인가)와 기간(몇 년 치가 있는가)을 확인하고, 웹 검색으로 찾은 것은 2.1에서 한 것처럼 링크를 직접 연다. 단위가 어긋나면 주제 자체를 바꿔야 할 수 있다. 확인이 끝나면 주제를 하나로 정하고, 메모 폴더에 '프로젝트계획서_6주차.md'를 다음 표 형식으로 작성한다. 이 문서는 이후 실습(7·9·13·14주차)에서 계속 꺼내 보고 고쳐 쓰게 된다.
...
## 2.11 선택 심화: 프로젝트 데이터 후보 두 개를 대조표로 비교해 고르기

**무엇을 왜 하는가.** 핵심 변수의 데이터 후보가 둘 이상 남았다면, 감으로 고르지 말고 2.3의 판정표를 두 후보에 나란히 적용한다. 같은 "인구"라도 주민등록 기준과 인구총조사 기준이 다르고, 같은 "출산율"이라도 시군구까지 있는 표와 시도까지만 있는 표가 있다. 이 단계는 수업 시간에 못 해도 된다. 프로젝트 데이터를 확정하기 전에 집에서 하면 된다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"내 프로젝트의 핵심 변수 후보 두 개, (가) 시군구별 주민등록 총인구, (나) 시군구별 합계출산율에 대해 kosis MCP의 검색 도구와 구조 확인 도구로 통계표 이름과 ID, 작성 기관, 분석 단위(시도까지인지 시군구까지인지), 수록 기간과 주기, 단위를 확인해서 두 후보를 같은 항목으로 나란히 놓은 대조표로 정리해 줘. MCP로 확인되지 않는 항목(이용허락, 갱신 이력)은 '화면 확인 필요'라고 적고, 추측으로 채우지 마. 어느 쪽이 낫다는 판단은 하지 말고 표만 줘."
</div>

**예상되는 에이전트의 행동과 결과.** 후보마다 검색 도구와 구조 확인 도구가 한 번씩 호출되고, 표 6-11과 같은 대조표가 온다. (가)의 칸은 오늘 확인한 사실로 채워질 것이고, (나)의 칸 중 이 교재가 확인해 적지 않은 것은 여러분 화면의 값으로 채운다.

**표 6-11. 데이터 후보 대조표 (예: 시군구 인구 vs 시군구 합계출산율)**

| 확인 항목 | (가) 시군구 주민등록 총인구 | (나) 시군구 합계출산율 |
|------|------|------|
| 통계표와 ID | 행정구역(시군구)별 성별 인구수, DT_1B040A3 | (검색 결과의 표 이름과 ID를 적는다) |
| 작성 기관과 원천 | 행정안전부 주민등록 자료, KOSIS 수록 | 국가데이터처 인구동향조사 (출생 신고 기반) |
| 분석 단위 | 전국·시도·시군구 | 시군구까지 있음 (4주차 sigungu_2023.csv의 원천) |
| 수록 기간과 주기 | 연도별. 2019-2023년 확인 | (화면에서 확인) |
| 단위 | 명 | 명 (여성 1명당 출생아 수) |
| 이미 아는 문제 | 시군구 조회 시 계층 지정 필요 | 2023년 경북 군위군 결측 (대구 편입) |
| 이용허락과 갱신 이력 | 화면 확인 필요 | 화면 확인 필요 |

**검증.** 두 후보를 각각 KOSIS 화면에서 열어, 대조표의 분석 단위와 수록 기간 칸을 지역 선택 창과 시점 선택 창으로 대조하고, "화면 확인 필요"로 남은 칸을 직접 채운다. 그리고 결정을 내린다. 어느 후보가 내 분석 질문의 단위와 기간에 맞는가, 둘을 함께 써야 한다면 병합 키(시도와 시군구 이름)가 서로 맞는가. 후자는 7주차에서 실제로 확인하게 되며, 표 6-11의 "이미 아는 문제" 행이 그 예고다. 결정했으면 프로젝트계획서_6주차.md의 "필요한 데이터" 칸을 고치고 고른 이유를 한 줄 덧붙인다. 대조표를 만든 것은 에이전트지만 읽고 고른 것은 여러분이라는 사실이 계획서에 남아야 한다.
...
- [ ] 등록 범위를 확인해 사용자 범위로 옮겼고, 다른 폴더를 열어도 `/mcp`에 kosis가 보이는 것을 확인했다
- [ ] 틀린 주소로 등록해 보고 "등록 성공"과 "연결됨"이 다른 말임을 확인한 뒤 실험용 서버를 지웠다
- [ ] 2.3에서 고른 통계표의 항목·분류·기간을 KOSIS 화면과 대조하고 표 6-4를 채웠다
- [ ] 2.4에서 세 검색어의 결과를 한 표에 나란히 놓았고, DT_1B040A3이 어느 검색어에서 나왔는지 표시했으며, 검색 호출이 세 번이었음을 로그에서 확인했다
...
- [ ] 표 6-5의 세 값(전국·서울·세종)을 다섯 걸음의 화면 절차로 직접 찾아 채웠고, 화면의 단위 표기와 시점 표기도 함께 적었다
...
- [ ] 2.7의 다섯 해 조회가 5건임을 확인했고, 안내문이 원문 그대로 붙어 있는지 보았다
- [ ] 표 6-8의 증감 네 칸과 누적을 손으로 계산하고, 증감의 합이 누적과 같음을 확인한 뒤 에이전트의 계산과 대조했다
- [ ] 값 아래에 붙은 기준 시점 설명의 출처를 확인했고, 확인되지 않은 설명은 "확인 필요"로 남겼다
- [ ] 수집기록.md에 두 건이 같은 양식으로 적혔다 (표 6-7, 표 6-9). 두 번째 기록의 검증 내용에 손 검산 결과와 재지시 이력이 들어 있다
- [ ] `/csv-profile` 보고서의 행 수(17)·최댓값·평균(3,019,137)을 파일과 손 계산으로 대조했고, 연도 열 경고에 대한 판단을 적었다
- [ ] 2.9의 kosis-collect 스킬을 만들어 실행했고, 스킬이 저장한 다섯 값이 손 검산 값과 한 자리도 다르지 않음을 확인했다
- [ ] 없는 통계표 ID로 스킬을 불러 보고, 조회를 건너뛰거나 다른 표로 바꾸지 않고 멈추는지 확인했다
...
| 등록은 성공했다는데 연결 상태가 계속 실패다 | 주소의 경로에 오타가 있다 (2.2의 실험과 같은 증상) | 오류 메시지에 다시 적힌 주소를 내가 준 주소와 한 글자씩 대조한다. 등록 성공 메시지는 주소를 적어 두었다는 뜻일 뿐이다 |
...
| 검색어를 바꿨더니 다른 통계표가 나와 어느 쪽이 맞는지 모르겠다 | 검색어의 넓이가 결과를 정한다 (2.4) | 표 6-4의 판정표를 두 후보에 각각 적용해 비교한다. 한 번 고른 표는 이름이 아니라 ID로 부른다 |
| 세 검색어를 따로 검색하라고 했는데 도구를 한 번만 호출했다 | 첫 결과에서 나머지를 추려 냄 | 로그의 호출 횟수를 세어 확인하고, "검색어마다 도구를 따로 호출해서 결과를 보여 줘"라고 재지시 |
...
| `/csv-profile` 보고서가 연도 열을 품질 경고로 잡는다 | 4주차 SKILL.md의 "값이 하나뿐인 열" 규칙이 정상 작동한 것 | 오류가 아니다. 한 해만 수집한 설계의 결과임을 "확인 필요" 절에 판단으로 적는다 |
| 주제는 정했는데 시군구 데이터가 없고 시도 단위만 있다 | 통계의 공표 단위 제약 | 주제를 시도 단위로 조정하거나, 시군구 단위가 있는 인접 지표로 대체. 결정 전에 교수자와 상의 |
| `/kosis-collect`가 자동완성 목록에 보이지 않는다 | 폴더 위치나 파일 이름이 다름 | `.claude/skills/kosis-collect/SKILL.md` 경로를 확인한다 (4주차에서 한 점검과 같다) |
...
**과제 6-D. 추이 수집과 손 검산 세트.** 2.7과 2.8의 산출물을 묶어 제출한다. (1) data/korea_pop_2019_2023.csv, (2) 손으로 채운 표 6-8과 에이전트의 증감 계산을 대조한 결과, (3) 수집기록.md의 두 번째 항목, (4) reports/프로파일_sido_pop_2023.md와 그 "확인 필요" 절에 적은 자신의 판단 문장. 평가 기준은 손 검산의 정확성과, 수집 중 재지시가 있었다면 그 원인(기간 누락, 시군구 섞임, 안내문 누락 등)을 기록으로 남겼는가이다. 재지시가 한 번도 없었다면 "왜 없었는가"를 지시문의 손잡이와 연결해 한 문단으로 쓴다.

**과제 6-E. 수집 절차의 스킬화와 재현 시험.** 2.9에서 만든 `.claude/skills/kosis-collect/` 폴더(SKILL.md와 참조 파일)와, 그 스킬로 다시 수집한 산출물, 그리고 시험 기록을 묶어 제출한다. 평가 기준은 넷이다. (1) 무엇을 인자로 뺐고 무엇을 절차에 고정했는지, 그 기준을 자기 문장 한 문단으로 설명했는가. (2) 스킬이 저장한 다섯 값이 손으로 검산한 값과 같은가. (3) 없는 통계표 ID를 넣은 오류 시험에서 스킬이 다른 표로 바꾸지 않고 멈추었는가, 멈추지 않았다면 SKILL.md의 어느 줄을 어떻게 고쳤는가. (4) 스킬 없이 같은 요청을 한 기준선 비교에서 무엇이 달랐는지 두 줄로 적었는가. 이 스킬은 14주차 종합 프로젝트에서 자신의 데이터에 그대로 쓰게 되므로, 제출 후에도 폴더에 남겨 둔다.
