...
진행은 세 단계다. 앞부분(2.1-2.3)은 설치다. 저장소를 먼저 살펴보고, 마켓플레이스를 등록해 플러그인을 설치하고, 설치된 스킬의 SKILL.md를 열어 4주차에 손으로 만든 파일과 같은 구조임을 확인한다. 가운데(2.4-2.5)는 사용이다. 예시 공문서를 Markdown으로 바꾸고, 그 안의 표를 CSV로 옮겨 pandas로 읽는다. 뒷부분(2.6-2.7)은 관찰과 검증이다. 스킬 이름을 부르지 않아도 에이전트가 스스로 꺼내 쓰는지 보고, 변환 결과가 원문과 같은지 사람이 직접 대조한다. 마지막의 2.8은 과제 안내다. 엑셀 서식 3,103장에서 법인별 재무 항목을 모아 과의 양식대로 hwpx 보고서를 만드는 과제를 소개하고, 수업에서는 풀 실마리를 다룬다.
...
**반드시 지킬 것과 나머지.** 반드시 지켜야 하는 것은 둘뿐이다. 하나는 양식의 다섯 절(개요, 현황, 문제점, 시사점, 붙임)과 현황의 표 두 개를 모두 채우는 것이고, 다른 하나는 만든 hwpx 파일이 한글이나 한컴독스에서 깨지지 않고 열리는 것이다. 그 밖의 세부 기준은 두지 않는다. 그림 5-5와 비슷한 것이 나오면 된다. 이 보고서의 품질은 여러분이 에이전트와 얼마나 잘 소통했는지에 달려 있다. 무엇을 어느 칸에서 뽑을지, 어떤 파일을 뺄지, 어떤 기준으로 중복을 정리할지를 분명히 말해 주고, 나온 결과를 확인해 되묻는 만큼 보고서가 좋아진다.
...
| 만든 hwpx가 한글에서 열리지 않거나 표가 깨진다 | 생성 도구의 산출물 문제이거나 표가 너무 넓다 | 한글이나 한컴독스에서 열어 확인한 뒤 열 수를 줄이거나 다른 프리셋으로 다시 만든다. 열리지 않는 파일은 결과물이 아니다 |
| 예시 문서의 내용을 실제 행정 사실로 인용하고 싶어진다 | 문서의 겉모습이 공문서와 같다 | 이 문서는 수업용으로 만든 가상의 예시다. 인용하지 않는다. 안의 수치만 원본 데이터와 같다 |

## 혼자서 해 보는 실습

**혼자 해 보기 5-A. 설치 점검과 변환 산출물.** 오늘 실습의 결과를 묶어 정리한다. 남길 것은 넷이다. (1) 2.1에서 확인한 것을 자기 문장으로 다시 쓴 한 문단. 누가 만들었고, 무엇을 하는 도구이며, 내 컴퓨터에서 무엇을 실행하고, 라이선스가 무엇인지가 들어가야 한다. (2) `data/docs/인구현황보고_예시.md`. (3) `data/인구상위5.csv`. (4) 2.7의 대조 결과와, 그중 자신이 파일을 직접 열어 눈으로 확인한 값 하나를 밝힌 한 문장. 스스로 확인할 것은 두 가지다. 설치 전 확인이 남의 말이 아니라 자신이 확인한 것으로 적혀 있는가, 그리고 대조에서 사람이 직접 본 지점이 구체적으로 드러나는가.
