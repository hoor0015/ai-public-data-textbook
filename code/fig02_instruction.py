...
fig.savefig(FIG / "fig02_five_elements.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(FIG / "fig02_bad_vs_good.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
ax.text(6.5, 0.4, "실행 전에 사람이 계획을 검토하는 관문이 생긴다. 잘못된 방향이면 파일이 바뀌기 전에 잡는다.",
        ha="center", fontsize=10.5, color="#333")
fig.savefig(FIG / "fig02_plan_mode.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(FIG / "fig02_independent_check.png", dpi=150, bbox_inches="tight")
plt.close(fig)
