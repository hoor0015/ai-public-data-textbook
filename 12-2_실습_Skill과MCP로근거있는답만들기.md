...
그림 12-4가 완성된 인용 검증표의 모습이다. 읽는 법은 왼쪽에서 오른쪽이다. 왼쪽 두 열은 에이전트가 적은 것(주장과 근거 위치)이고, 오른쪽 두 열은 사람이 원문을 확인해야만 채워지는 것(확인 결과와 판정)이다. 세 행이 각각 다른 판정을 보여 준다. 첫 행은 위치도 문장도 맞는 일치, 둘째 행은 문장은 있는데 위치 표기가 틀린 위치 어긋남, 셋째 행은 그 문장 자체가 원문에 없는 경우다. 여기서 알 수 있는 것은 검증의 결과가 맞음과 틀림의 둘이 아니라 셋이라는 점이다. 위치 어긋남은 내용이 맞으므로 위치만 고치면 되지만, 원문에 없음은 그 주장을 메모에서 빼야 하는 사유다. 둘을 뭉뚱그려 "조금 틀렸다"고 적으면 무엇을 고쳐야 하는지 알 수 없다.
