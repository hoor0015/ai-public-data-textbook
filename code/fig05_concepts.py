...
# 왼쪽: GitHub 저장소 (컨테이너이므로 머리글은 상자 위쪽에 그대로 둔다)
...
box(ax, 0.7, 6.55, 4.9, 1.3,
    "마켓플레이스 파일\n.claude-plugin/marketplace.json\n이 저장소가 내놓는 꾸러미의 목록",
    fc=NAVY_F, ec=NAVY)

# 플러그인 상자도 컨테이너다 (안에 스킬 상자가 들어 있다)
...
box(ax, 1.15, 2.95, 4.0, 1.9,
    "스킬 kordoc\nskills/kordoc/SKILL.md\n4주차에 직접 만든 파일과 같은 구조\n(머리말 + 절차)",
    fc=GREEN_F, ec=GREEN)
...
# 설치 자리 상자도 컨테이너다 (안에 SKILL.md 사본 상자가 들어 있다)
...
box(ax, 9.9, 4.85, 3.9, 1.25,
    "SKILL.md 사본\n경로에 버전이 들어 있으므로\n내 원본이 아니라 특정 판의 사본이다",
    fc=GREEN_F, ec=GREEN)

box(ax, 9.4, 1.5, 4.9, 2.35,
    "대화창에서 부른다\n/kordoc:kordoc\n(플러그인 이름 : 스킬 이름)\n"
    "이름을 부르지 않아도 요청이 설명과\n맞으면 에이전트가 스스로 쓴다",
    fc=BLUE_F, ec=BLUE)

# 가운데 화살표 두 개 (상자 밖 텍스트이므로 크기를 그대로 둔다)
...
for x, y, ec, fc, title, lines, ex in cards:
    ...
    # 카드의 이름표는 한 덩어리로 합쳐 figfit이 크기를 정하게 한다
    ax.text(x + 3.2, y + 1.95, title + "\n" + lines[0] + "\n" + lines[1],
            ha="center", va="center")
    # kordoc의 답은 카드 안에 얹은 예시 주석이므로 그대로 둔다
...
box(ax, 0.6, 8.6, 6.0, 1.0, "반복되는 절차 하나가 생겼다", fc=NAVY_F, ec=NAVY)

qs = [
    (6.55, "질문 1. 같은 일을 하는\n공개 스킬이 이미 있는가"),
    (4.65, "질문 2. 설치 전\n네 가지 점검을 통과하는가"),
    (2.75, "질문 3. 우리 기관의 기준과 판단이\n절차에 들어가는가"),
]
for y, q in qs:
    box(ax, 0.6, y, 6.0, 1.25, q, fc="white", ec=BLUE)
...
for y, ec, fc, t1, t2 in outs:
    box(ax, 8.4, y, 5.0, 1.25, t1 + "\n" + t2, fc=fc, ec=ec)

box(ax, 0.6, 0.5, 6.0, 1.25,
    "가져다 쓴다\n설치하고, 결과는 사람이 원문과 대조한다", fc=BLUE_F, ec=BLUE)
