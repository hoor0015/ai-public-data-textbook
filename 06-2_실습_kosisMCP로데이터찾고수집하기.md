...
스킬 이름을 대지 않고 "DT_1B040A3에서 2019-2023년 전국 총인구를 kosis로 받아 CSV와 수집기록으로 남겨 줘"처럼 평범한 문장으로 부탁해도 된다. 4주차에서 본 대로 머리말의 `description`이 자동 호출의 판단 근거이기 때문이다. 어느 쪽으로 부르든 에이전트가 SKILL.md의 절차를 순서대로 밟는 모습이 도구 사용 로그에 나타난다. 아래는 그 로그의 모양이다. 표시 방식은 버전에 따라 다르지만 읽을 것은 같다.
...
① 산출물 대조. 스킬이 저장한 CSV의 다섯 값이 2.6에서 손으로 검산해 둔 값과 한 자리도 다르지 않은지 본다. 스킬이 새로 쓴 수집기록 항목도 앞의 두 기록과 같은 항목이 모두 있는지 훑고, 검증 내용 칸이 비어 있는지 확인한다. 채워져 있다면 스킬이 절차 6의 마지막 문장을 어긴 것이므로 그 문장을 더 분명하게 고친다.
...
- [ ] 2.8의 kosis-collect 스킬을 만들어 실행했고, 스킬이 저장한 다섯 값이 손 검산 값과 한 자리도 다르지 않음을 확인했다
