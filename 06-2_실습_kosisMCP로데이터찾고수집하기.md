...
**과제 6-D. 수집 절차의 스킬화.** 2.8에서 만든 `.claude/skills/kosis-collect/` 폴더(SKILL.md와 참조 파일)와 그 스킬로 수집한 `data/sido_pop_2019_2023.csv`, 그리고 스킬이 덧붙인 수집기록 항목을 제출한다. 평가 기준은 둘이다. (1) 무엇을 절차에 고정했고 무엇을 부를 때 말로 일러 주기로 했는지, 그 기준을 자기 문장 한 문단으로 설명했는가. (2) 파일이 85행이고, 2023년 값이 과제 6-A의 파일과 일치하며, 연도별 시도 합계가 2.6의 전국 값과 맞는가. 이 스킬은 14주차 종합 프로젝트에서 자신의 데이터에 그대로 쓰게 되므로, 제출 후에도 폴더에 남겨 둔다.
