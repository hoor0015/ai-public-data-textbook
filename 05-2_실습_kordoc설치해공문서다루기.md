...
**실마리 7. 도구는 자유롭게 고른다.** 오늘 설치한 kordoc에는 문서를 읽는 기능 말고도 Markdown 글을 공문서 모양의 hwpx로 만들어 주는 기능이 있고, 보고서와 계획서 같은 프리셋이 있다. 양식 파일을 kordoc으로 읽으면 네 절의 짜임과 표의 열 이름이 그대로 보이므로, 그 짜임대로 Markdown을 쓰고 hwpx로 만드는 길이 가장 짧다. 다른 길도 열려 있다. 마켓플레이스에서 엑셀이나 한글 문서를 다루는 다른 스킬을 찾아 설치해도 되고, 4주차처럼 이 절차를 내 스킬로 만들어 두어도 된다. 다만 무엇을 설치하든 1회차 1.3의 네 가지 확인을 먼저 하고, 어떤 도구로 무엇을 했는지 스스로 알고 있어야 한다. 도구가 만든 hwpx는 한글이나 한컴독스에서 열어 눈으로 확인한다. 열리지 않거나 표가 깨진 파일은 결과물이 아니다.

**최종 결과는 이런 모습이다.**

![그림 5-5. 과제의 최종 결과 예시: 양식대로 만든 보고서 hwpx의 두 쪽](figures/fig05_gongik_report.png)

그림 5-5를 읽어 보자. 왼쪽이 첫 쪽이다. 제목 아래 음영 상자가 요약이고, 로마 숫자 띠가 양식의 네 절이다. 오른쪽 둘째 쪽에 현황의 표 두 개가 채워져 있는데, 첫 표에서 평균과 중앙값의 열이 나란히 놓여 그 차이가 한눈에 보이고, 둘째 표에는 같은 법인이 두 번 나오지 않는다. 문제점 절의 두 항목은 모두 개수가 붙어 있다. 여기서 알 수 있는 것은 이 보고서의 값어치가 숫자의 크기가 아니라 "어떤 기준으로 세었는가"가 적혀 있다는 데 있다는 점이다. 여러분의 숫자는 이 그림과 조금 다를 수 있다. 중복을 정리하는 기준이나 빈 파일을 다루는 방식이 다르면 법인 수부터 달라지기 때문이고, 그 기준을 개요에 적어 두었다면 다른 숫자도 맞는 답이다.

**반드시 지킬 것과 나머지.** 반드시 지켜야 하는 것은 둘뿐이다. 하나는 양식의 네 절(개요, 현황, 문제점, 시사점)과 현황의 표 두 개를 모두 채우는 것이고, 다른 하나는 만든 hwpx 파일이 한글이나 한컴독스에서 깨지지 않고 열리는 것이다. 그 밖의 세부 기준은 두지 않는다. 그림 5-5와 비슷한 것이 나오면 된다. 이 보고서의 품질은 여러분이 에이전트와 얼마나 잘 소통했는지에 달려 있다. 무엇을 어느 칸에서 뽑을지, 어떤 파일을 뺄지, 어떤 기준으로 중복을 정리할지를 분명히 말해 주고, 나온 결과를 확인해 되묻는 만큼 보고서가 좋아진다.
