...
- "에이전트가 무엇을 하고 무엇을 못 하는지"를 다섯 가지 시험으로 확인한 경험
...
**검증.** 시험별로 확인할 것이 다르다. 시험 3은 도구 사용 기록에 검색이 있는가, 답에 출처와 시각이 있는가를 본다. 시험 4는 탐색기의 data 폴더가 비어 있음을 여러분이 알고 있으므로, 에이전트의 답이 그 사실과 맞는가를 본다. 시험 5는 코드가 실제로 실행되었는지 터미널 또는 도구 기록에서 확인하고, 두 답이 다르다면 코드 답을 기준으로 삼는다. 다섯 시험에서 무엇을 보아야 하는지는 표 1-3에 모아 두었다.
...
**과제 1-B. 여섯 번째 시험.** 2.7절의 다섯 시험 외에 에이전트의 한계를 건드리는 시험을 하나 직접 설계해 실제로 해 보고, 입력한 지시문과 그 답을 무엇으로 확인할 수 있는지를 세 문장 안으로 적어 제출한다(예: 오늘 날짜 묻기, 어제 만든 파일 찾게 하기, 긴 문서 요약시키기). 평가 기준은 시험의 독창성이 아니라, 확인 방법이 화면과 파일에서 실제로 밟을 수 있는 절차로 적혀 있는가이다.
