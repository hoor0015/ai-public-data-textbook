...
1회차에서 Skill(반복 절차의 표준화)을 원리로 배웠다. 오늘은 직접 만들어 본다. 1회차에서 배운 개념은 그것이 필요한 단계에서 화면에 나온 결과를 놓고 다시 짚는다. 단계마다 있는 "먼저 알아 둘 것"과 "짚고 가기"가 그 자리다. 재료는 앞으로 학기 내내 반복하게 될 작업, 곧 데이터 프로파일링이다. 새 데이터를 받으면 분석보다 먼저 신상명세(행·열·자료형·결측·요약통계)를 확인해야 하는데, 이 진단 절차가 전형적인 반복 작업이다.
...
**검증.** 2주차와 3주차에서 익힌 대로 결정적 값 셋을 원본에서 확인한다. 행 수(마지막 데이터 줄이 230번째 줄이므로 229행), 결측 위치(73번째 줄 군위군의 끝이 쉼표 두 개로 비어 있다), 그리고 보고된 최댓값 하나(예: 32번째 줄 수원시의 총인구 1190964.0)다.

검증 중에 발견하게 되는 미묘한 함정이 하나 있다. 에이전트가 "합계출산율의 평균은 0.823"이라고 보고했다면 이 평균은 몇 개 값의 평균인가. 229개가 아니라 군위군을 뺀 228개다. pandas는 평균을 낼 때 결측값을 조용히 제외하며, 에이전트도 그 사실을 따로 밝히지 않고 넘어가는 일이 많다. 위의 보고서 예시에서도 3절에 결측이 적혀 있을 뿐, 4절의 평균이 228개로 계산되었다는 말은 어디에도 없다. 에이전트에게 "이 평균은 몇 개 값으로 계산했어?"라고 되물어 보라. 이런 되묻기가 2주차에서 배운 검증 내장 지시의 실전형이다.

**짚고 가기: 이 지시문을 다음 주에도 친다면.** 방금 입력한 지시문은 다시 쓰게 될 지시문이다. 6주차에 새 데이터를 수집하면 이 지시를 또 치게 되고, 그다음 주에도 친다. 그때마다 지난번 지시문을 찾아 복사해 붙이는 방식에는 세 가지 비용이 든다. 첫째는 번거로움이다. 방금 지시문은 다섯 항목에 저장 위치까지 적은 긴 글이어서, 찾아서 붙이고 파일 이름을 고치는 데 매번 1-2분이 든다. 둘째는 표류다. 급한 날 기억나는 대로 다시 치다가 (3)번 항목의 "결측이 있는 행이 정확히 어느 시군구인지"를 빠뜨렸다고 하자. 그날의 보고서에는 결측 개수 1만 적히고 군위군이라는 이름은 나오지 않는다. 지시가 달라지면 산출물이 달라지고, 형식이 들쭉날쭉한 보고서는 서로 비교할 수 없다. 셋째는 나만 안다는 것이다. 지시문이 내 메모 파일에 있는 한 에이전트는 그런 지시문이 있는 줄 모르고, 옆자리 동료도 모른다.

세 비용을 한 번에 줄이는 방법은 지시문을 사람의 메모가 아니라 에이전트가 아는 자리에 정해진 형식으로 저장하는 것이다. 행정 조직이 민원 접수 절차나 보도자료 작성 요령을 업무 매뉴얼로 만들어 담당자가 바뀌어도 품질을 유지하는 것과 같은 발상이고, Claude Code에서는 그 자리가 Skill이다. 모든 지시를 스킬로 만들 필요는 없다. 절차가 여러 단계이고, 산출물 형식이 정해져 있고, 주기적으로 반복되면 스킬 후보다. 방금 한 프로파일링은 다섯 단계이고, 보고서의 절이 정해져 있고, 새 파일을 받을 때마다 반복되므로 세 조건을 모두 채운다. 반대로 "방금 만든 보고서의 제목을 바꿔 줘"처럼 한 번만 할 일이나 "표의 숫자에는 천 단위 쉼표"처럼 절차가 아니라 취향인 것은 후보가 아니다. 같은 지시를 세 번째 치고 있다면 저장할 때이고, 여러분은 지금 그 직전에 있다. (자세한 설명은 1회차 1.1, 1.3)

## 2.2 SKILL.md 작성: csv-profile

**무엇을 왜 하는가.** 2.1의 절차를 SKILL.md라는 파일로 저장한다. 한 번 저장해 두면 다음부터는 긴 지시문 대신 명령 한 줄로 같은 절차를 부를 수 있다.

**먼저 알아 둘 것.** Skill은 폴더 하나와 그 안의 SKILL.md 파일 하나로 이루어진다. 특별한 프로그램이나 설정 화면이 있는 것이 아니라, 정해진 자리에 놓인 텍스트 파일이 전부다.
...
<strong>정의 · Skill과 SKILL.md</strong><br>
<strong>Skill</strong>(스킬)은 에이전트에게 특정 작업의 절차를 가르쳐 두는 확장 장치다. 폴더 하나에 <strong>SKILL.md</strong>라는 지시문 파일을 넣어 만들며, 프로젝트 폴더의 <code>.claude/skills/스킬이름/SKILL.md</code>에 두면 그 프로젝트에서, 사용자 홈 폴더 아래의 <code>.claude/skills/스킬이름/SKILL.md</code>에 두면 모든 프로젝트에서 쓸 수 있다. 에이전트가 관련 요청을 받으면 스스로 불러 쓰기도 하고, 사용자가 슬래시 커맨드(2.3)로 직접 부를 수도 있다.
</div>

2주차에 만든 CLAUDE.md에 이 절차를 적어 넣으면 되지 않느냐고 물을 수 있다. 그렇게 하지 않는 까닭은 CLAUDE.md가 매 대화에 통째로 로드되는 "항상 켜져 있는" 파일이기 때문이다. 프로파일링 절차는 새 CSV를 받은 날에만 필요한데, CLAUDE.md에 적어 두면 문서를 요약하는 날에도 환경을 점검하는 날에도 그 긴 절차문이 맥락창(2주차의 머리 비유에서 한정된 작업 기억)의 한 자리를 차지한다. 항상 적용되는 규칙과 특정 상황에서만 실행하는 절차는 성격이 달라서 담는 그릇도 다르다.

![그림 4-1. 일회성 지시, Skill, CLAUDE.md의 3층 구조](figures/fig04_three_layers.png)

그림 4-1이 세 그릇의 관계다. 맨 위의 일회성 지시는 이번 한 번만 필요한 요청으로, 2.1에서 입력한 지시문이 여기에 해당한다. 맨 아래의 CLAUDE.md는 "원본 데이터는 data 폴더에 있고 수정하지 않는다" 같은 항상 적용되는 규칙이다. Skill은 그 사이에 있어서, 반복해서 쓰지만 항상 필요하지는 않은 절차를 담아 두고 부를 때만 로드한다. 오늘 할 일은 2.1의 지시문을 맨 위층에서 가운데 층으로 내리는 것이다.

SKILL.md 파일은 두 부분으로 이루어진다. 파일 맨 위에 `---` 두 줄 사이로 끼워 넣는 머리말(frontmatter)과 그 아래의 본문이다. 머리말에는 스킬의 이름(`name`)과 설명(`description`)을 적고, 본문에는 에이전트가 따를 절차를 적는다. 본문의 형식은 자유롭지만 잘 만든 본문은 2주차에서 배운 좋은 지시문과 똑같이 생겼다. 단계가 순서대로 나뉘어 있고, 산출물의 형식이 명시되어 있고, 성공 기준이 들어 있다. (자세한 설명은 1회차 1.2, 1.3)

이제 만든다. 먼저 프로젝트 폴더 안에 스킬 폴더를 만든다. 에이전트에게 "`.claude/skills/csv-profile/` 폴더를 만들어 줘"라고 시키면 되고, VSCode 탐색기에서 직접 만들어도 된다. 위치는 `.claude/skills/csv-profile/`이다 (`.claude` 폴더는 2주차에 CLAUDE.md를 만들며 이미 생겼을 것이다). 그 안에 `SKILL.md` 파일을 만들고 아래 내용을 그대로 입력한다.
...
이 SKILL.md를 뜯어 보자. 머리말의 `description`은 "무엇을 하는지"와 "언제 쓰는지"를 함께 적어, 에이전트가 자동 사용 여부를 판단할 근거를 준다. 본문 첫 줄은 대상 파일을 못 박지 않고 사용자가 말한 파일을 받겠다고 적어 둔 자리다. 파일마다 스킬을 고칠 필요가 없고, 분명하지 않으면 묻게 해 두어 엉뚱한 파일로 진행하는 일도 막는다. "절차"는 2.1에서 방금 수행한 단계들의 표준판이고, "산출물 형식"이 있어 매번 같은 꼴의 보고서가 나온다. 그리고 "규칙"의 마지막 항목이 사람의 확인 지점이다. 스킬은 판단하지 않고, 판단할 거리를 사람 앞에 모아 놓는다. 이 항목이 왜 필요한지는 2.6에서 실제 장면으로 보게 된다.
...
**짚고 가기: 에이전트는 평소에 이름과 설명만 안다.** 방금 에이전트가 알려 준 것은 스킬의 이름과 설명 두 가지뿐이고, 절차 일곱 단계는 말하지 않았다. 에이전트가 게을러서가 아니라 스킬이 원래 그렇게 작동한다. 에이전트는 평소에 각 스킬의 이름과 설명만 목록으로 갖고 있고, 본문은 그 스킬이 실제로 불릴 때에만 맥락창에 로드한다. 그래서 긴 절차와 참고자료를 스킬에 담아 두어도 쓰지 않는 동안에는 맥락창을 거의 차지하지 않는다. 규칙은 항상 켜 두고(CLAUDE.md) 절차는 서랍에 넣어 두었다가 꺼내 쓰는(Skill) 구조다. 이 구조에서 `description`이 맡는 역할이 크다. 서랍을 열지 않은 상태에서 에이전트가 볼 수 있는 것은 서랍 겉의 이름표뿐이므로, "지금 이 요청에 이 스킬을 쓸까"를 판단하는 근거가 설명 한두 문장이 전부다. 그래서 설명에는 무엇을 하는 스킬인지와 언제 쓰는지를 함께 적는다. 설명이 부실하면 어떤 일이 생기는지는 2.4에서 직접 겪는다. (자세한 설명은 1회차 1.2, 1.3)
...
"프로파일링 스킬 만들어 줘"라고 에이전트에게 통째로 맡길 수도 있고, 실제로 잘 만들어 준다. 그런데 첫 스킬만큼은 손으로 만들어 보길 권한다. 이유는 두 가지다. 첫째, SKILL.md가 신비한 설정 파일이 아니라 <strong>그냥 지시문을 적은 텍스트 파일</strong>임을 몸으로 확인하는 것이 중요하다. 둘째, 스킬의 절차와 규칙을 정하는 일은 2.6에서 다시 볼 "기준의 설정"에 해당한다. 무엇을 표준으로 삼을지는 사람이 정해서 적어 넣는 것이고, 그 감각은 직접 써 봐야 생긴다. 두 번째 스킬부터는 초안 작성을 에이전트에게 맡기고 여러분이 기준을 다듬는 분업이 효율적이다. 오늘 2.9(선택 심화)에서 그 분업을 해 본다.
</div>

## 2.3 실행과 검증: 슬래시 커맨드로 부르기

**무엇을 왜 하는가.** 스킬은 만들었다고 끝이 아니다. 스킬도 에이전트의 산출물처럼 검증 가능한 단위로 테스트한다. 첫 테스트는 직접 호출이다. 대화창에 `/csv-profile`을 입력해 스킬을 부르고, 이어서 어떤 파일을 볼 것인지 한 문장으로 적는다. 스킬 본문이 대상을 사용자가 말한 파일로 두었으므로, 이 한 문장이 그 자리를 채운다.
...
**짚고 가기: 빗금 하나가 한 일.** 방금 입력한 `/csv-profile`처럼 대화창에서 빗금과 이름으로 기능을 직접 실행하는 명령을 슬래시 커맨드라 부른다.

<div style="border:1px solid #cdd9e5;border-left:5px solid #2f6fb0;background:#f5f9fd;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>정의 · 슬래시 커맨드</strong><br>
<strong>슬래시 커맨드</strong>(slash command)는 대화창에서 <code>/이름</code> 형태로 입력해 기능을 직접 실행하는 명령이다. Claude Code에는 <code>/help</code>, 3주차에 본 <code>/compact</code>와 <code>/clear</code> 같은 내장 명령이 있고, 사용자가 만든 스킬도 폴더 이름 그대로 슬래시 커맨드가 된다. 즉 스킬을 하나 만들면 "에이전트가 알아서 쓰는 절차"와 "내가 직접 부르는 명령"이 동시에 생기는 셈이다.
</div>

![그림 4-2. Skill의 작동 흐름과 SKILL.md의 두 부분](figures/fig04_skill_flow.png)

그림 4-2를 방금 본 도구 사용 로그와 맞춰 읽어 보자. 왼쪽은 2.2에서 확인한 평소 상태로, 에이전트는 스킬들의 이름과 설명만 목록으로 갖고 있다. 가운데의 호출 방법 1이 방금 한 일이다. `/csv-profile`을 입력하는 순간 SKILL.md 본문이 맥락창에 로드되었고(오른쪽), 그래서 로그에 절차 1부터 7까지가 적힌 순서대로 나타났다. 보고서의 절 제목이 SKILL.md의 산출물 형식과 글자까지 같았던 것도 에이전트가 그 본문을 읽고 따랐기 때문이다. 명령 뒤에 붙인 "data/sigungu_2023.csv를 프로파일링해 줘"는 본문 첫 줄의 "사용자가 말한 CSV 파일" 자리를 채웠다. 대상 파일을 스킬에 못 박지 않고 대화에서 말해 주는 방식이어서 하나의 스킬을 파일마다 고치지 않고 재사용할 수 있다. 그림 가운데의 호출 방법 2, 곧 이름을 부르지 않았는데도 에이전트가 설명을 보고 스스로 스킬을 고르는 길은 다음 단계에서 시험한다. (자세한 설명은 1회차 1.4)

**검증.** 오늘은 비교 기준이 있다. 2.1에서 손으로 만든 보고서다. 행 수 229와 군위군 결측(결측 개수 1, 비율 0.44%)이 2.1의 결과와 같은지 보고, 산출물 형식의 다섯 절이 모두 있는지 본다. 5절 "확인 필요"는 비어 있으면 안 된다. 군위군 결측의 처리 방법은 스킬이 결정할 일이 아니므로 질문으로 남아 있어야 한다. 수치의 재검산은 2.8에서 새 대화에 맡긴다.

## 2.4 자동 호출 시험: 이름을 부르지 않고 요청하기

**무엇을 왜 하는가.** 테스트 2는 자동 호출이다. 그림 4-2의 호출 방법 2가 말하듯 스킬은 요청이 `description`과 맞으면 에이전트가 스스로 꺼내 쓴다. 이 두 번째 길이 실제로 열려 있는지 확인해야 한다. 6주차부터는 "이 파일 어떤 데이터야?"라고 무심코 묻는 일이 잦을 텐데, 그때마다 스킬이 나서 주어야 표준 형식의 보고서가 쌓인다. 새 대화를 열고, 이번에는 스킬 이름을 말하지 않고 요청한다.
...
**짚고 가기: 방아쇠를 누가 당기는가.** 자동 호출은 편리하지만 모든 스킬에 어울리는 것은 아니다. 기본값으로는 사용자와 에이전트 모두 스킬을 부를 수 있다. csv-profile은 에이전트가 알아서 꺼내 써도 해가 없다. 파일을 읽어 보고서를 하나 더 만들 뿐이기 때문이다. 하지만 "보고서를 제출 폴더로 복사하고 이전 버전을 지우는" 절차를 스킬로 만들었다고 하자. 이런 절차를 에이전트가 요청을 넓게 해석해 스스로 실행하면 곤란하고, 언제 실행할지는 사람이 정해야 한다. 이럴 때는 머리말에 `disable-model-invocation: true` 한 줄을 추가한다. 그러면 에이전트의 자동 사용이 막히고 사용자의 슬래시 커맨드로만 실행된다. 반대 방향의 설정(`user-invocable: false`)도 있어서, 에이전트만 참고하는 배경지식용 스킬을 만들 수도 있다. 오늘 만드는 스킬에는 둘 다 넣지 않는다. 기억해 둘 것은 자동화 장치를 만들 때 그 장치의 방아쇠를 누가 당길지도 함께 정한다는 점이고, 이것이 2.5와 2.6에서 다룰 자동화의 경계 문제의 축소판이다. (자세한 설명은 1회차 1.4)
...
**짚고 가기: 무엇을 맡기고 무엇을 남길 것인가.** 행·열 수 대조는 스킬에 맡기기 좋은 일이다. 어떤 일을 자동화해도 되는지는 네 가지 질문으로 가늠한다. 반복되는가, 성공 기준이 명확한가, 틀렸을 때 피해가 작은가, 규칙으로 적을 수 있는가. 행·열 수 대조는 보고서를 만들 때마다 반복되고, "일치"와 "불일치"로 성공 기준이 갈리고, 내부 검토용 보고서라 틀려도 피해가 작고, 절차 8처럼 두 문장으로 적을 수 있다. 네 질문에 모두 그렇다고 답할 수 있으니 크게 맡긴다. 반대로 "군위군의 결측을 어떻게 처리할 것인가"는 성공 기준이 명확하지 않고 맥락마다 답이 달라서, 스킬은 결정하지 않고 5절 "확인 필요"에 질문으로 남긴다. 네 질문 가운데 하나라도 걸리면 자동화하더라도 사람의 확인 지점을 절차 안에 심는다는 것이 원칙이다. 스킬은 사람을 배제하는 도구가 아니라 사람의 확인이 필요한 지점을 절차 안에 명시하는 도구로 쓸 때 가장 안전하다. (자세한 설명은 1회차 1.5, 표 4-1)

맡긴 일의 범위도 정확히 알아 두자. 파일을 읽을 때 줄이 깨져 행이 합쳐지거나 쉼표 개수가 어긋나면 행 수나 열 수가 원본과 달라지는데, 대조 절은 그런 읽기 오류를 스킬이 스스로 잡아 알리게 만든 장치다. 반대로 행 수가 맞아도 값이 틀릴 수 있고, 평균이 몇 개 값으로 계산되었는지는 대조 절이 말해 주지 않는다. 자동 검증은 사람의 검증을 대신하는 것이 아니라 단순 대조 하나를 덜어 주는 것이다.
...
오늘의 두 번째 틀리는 장면이 여기 있다. 수치형 요약의 첫 행을 보라. 연도의 평균이 2,017.0이고 표준편차가 3.894다. 계산은 정확하다. 2011부터 2023까지 13개 연도를 더해 13으로 나누면 2017이 맞다. 그러나 "연도의 평균"은 아무 의미가 없는 숫자다. 연도는 값이 아니라 각 행이 언제의 기록인지 알려 주는 이름표이고, 이런 열을 식별자(identifier)라 부른다. 시군구 이름이 문자였기 때문에 sigungu 파일에서는 이 문제가 드러나지 않았다. 연도가 숫자로 저장되어 있으니 스킬은 절차 4를 충실히 따라 평균을 냈고, 품질 경고도 없고, 대조 절도 일치이며, 확인 필요 절은 비어 있다. 형식은 완벽하고 내용의 한 줄이 틀렸다. 표준 절차는 표준적이지 않은 입력 앞에서 오류 메시지도 없이 조용히 틀린다. 이런 틀림은 스킬 안의 자동 대조가 잡지 못하고, 사람의 눈에만 잡힌다. 확인 필요 절이 비어 있는 것도 신호다. 판단할 거리를 하나도 남기지 않았다면 판단할 거리를 못 본 것일 수 있다.
...
**짚고 가기: 끝까지 사람에게 남는 세 가지.** 작업을 아무리 잘게 자동화해도 스킬에 넘길 수 없는 일이 세 가지 남으며, 방금 장면에 셋이 모두 들어 있었다. 첫째는 기준의 설정이다. "식별자 열은 요약통계에서 제외한다"는 기준은 스킬이 세운 것이 아니라 여러분이 세워 SKILL.md에 적어 넣은 것이다. 품질 경고의 문턱을 결측 30%로 둔 것도 2.2에서 사람이 정했다. 스킬은 기준을 실행할 뿐 기준을 세우지 못한다. 둘째는 예외의 판단이다. 연도의 평균 2,017.0이 무의미하다는 것을 알아챈 것은 보고서를 읽은 사람이고, 알아챈 예외를 반영해 스킬을 고친 것도 사람이다. 셋째는 결과에 대한 책임이다. 이 보고서를 그대로 상사에게 냈다면 "연도 평균 2,017"이 적힌 문서에 이름을 올린 사람은 스킬이 아니라 여러분이다. 자동화는 작업 시간을 줄여 줄 뿐 검증 의무를 줄여 주지 않으며, 산출물이 늘어나는 만큼 검증할 것도 늘어난다. (자세한 설명은 1회차 1.5)

**검증.** 고친 뒤의 보고서에서 연도가 요약표 밖으로 나갔는지, 범위가 2011-2023으로 적혔는지 본다. 13행짜리 파일이라 지니계수의 최댓값 0.387(2011년)과 최솟값 0.323(2023년)은 파일을 열면 바로 보인다. 덧붙여 지니계수가 2011년 0.387에서 2023년 0.323으로 0.064 내려갔다는 사실을 기억해 두자. 이 하락이 얼마나 큰 것인지, 세로축을 어떻게 잡으면 크게 또는 작게 보이는지가 9주차 시각화의 주제다.

<div style="border:1px solid #ecd9c6;border-left:5px solid #c77b2f;background:#fdf9f4;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>유의 · 스킬이 있어도 검증은 사라지지 않는다</strong><br>
오늘 테스트를 통과한 스킬이 다음 달 새 파일에서도 맞으리라는 보장은 없다. 같은 형식의 보고서가 매번 말끔하게 나오기 시작하면 사람은 점점 안을 들여다보지 않게 된다. 자동 시스템의 산출물을 과신하는 이 경향을 자동화 편향(automation bias)이라 부른다. 연도의 평균 2,017.0은 표 안에 얌전히 앉아 있었고, 대조 절은 "일치"라고 안심시켰다. 스킬 산출물에 대한 최소 검증(행 수 대조, 수치 하나 재검산, 확인 필요 절이 비어 있는지 보기)을 여러분의 검증 체크리스트에 상시 항목으로 넣어 두라.
</div>
...
**검증.** 행 수는 이제 6절의 자동 대조(265줄, 264행)가 확인해 주므로 다시 세지 않는다. 사람이 볼 것은 두 가지다. 파일에서 "군위군"을 찾아 228번째 줄에 1.199가 채워져 있는지 확인하고, 세 보고서의 절 제목 여섯 줄이 글자까지 같은지 훑는다. 한 보고서만 제목이 다르면 그 실행에서 산출물 형식이 지켜지지 않은 것이므로, 어느 단계에서 어긋났는지 되물어 확인한다.
...
**짚고 가기: 지시문 라이브러리와 Skill은 무엇이 다른가.** Skill은 2주차에 시작한 지시문 라이브러리의 진화형이며, 차이는 세 가지다. 첫째, 복사해 붙여 넣는 일이 사라진다. 방금 본 대로 명령 한 줄이면 된다. 둘째, 에이전트도 라이브러리를 읽는다. 메모 파일 속의 템플릿은 여러분이 복사해 주어야만 쓰였지만, 스킬은 "합계출산율이 가장 높은 곳과 낮은 곳이 어디야?"라고만 물어도 에이전트가 설명을 보고 꺼내 쓴다. 셋째, 공유가 된다. 프로젝트 폴더의 `.claude/skills/`를 팀원에게 통째로 주면 팀 전체가 같은 절차로 일한다. 2.1에서 본 세 가지 비용(번거로움, 표류, 나만 안다)이 이 세 차이로 하나씩 풀린 셈이고, 행정 조직으로 치면 개인의 노하우가 업무 매뉴얼로 표준화되는 과정과 같다. (자세한 설명은 1회차 1.4)

**검증.** 결정적인 확인은 하나다. 73번째 줄의 군위군이 두 표 어디에도 없는지, 첫머리의 제외 보고에 그 줄이 적혀 있는지 본다. 결측을 0으로 읽었다면 군위군이 하위 1위로 올라와 있을 것이므로, 하위 표의 첫 줄이 부산 중구인지가 곧 결측 처리가 옳았는지의 답이 된다. 마지막으로 `code/`에 저장된 계산 코드를 열어 `fillna(0)` 같은 줄이 없는지 훑는다.

**사람의 확인 지점.** 이 스킬에서 사람이 판단해야 할 것은 순위에서 빠진 결측 행을 그대로 둘 것인가, 다른 출처로 값을 채운 뒤 순위를 다시 낼 것인가이며, 스킬은 이 질문을 "3. 확인 필요" 절에 남기기만 하고 결정하지 않는다. 이 분업(초안은 에이전트, 기준과 확인 지점은 사람)을 혼자 해 보기 4-A에서 자기 작업에 적용한다.
...
- [ ] 2.1의 수동 프로파일링에서 결정적 값 셋(229행, 73번째 줄 군위군 결측, 최댓값 하나)을 원본에서 확인했다
...
**혼자 해 보기 4-A. 나만의 두 번째 스킬.** 자신이 반복하는 작업 하나를 골라 스킬로 만들어 본다. 이번에는 2.9에서 해 본 분업을 써 보라. 초안 작성은 에이전트에게 맡기고("이런 작업을 하는 SKILL.md 초안을 만들어 줘"), 여러분은 절차와 기준을 다듬는다. 2.9의 top-bottom을 그대로 써도 되지만, 자기 작업(또는 2주차 지시문 라이브러리의 다른 항목)을 고르기를 권한다. 남길 것은 넷이다. (1) SKILL.md 전문, (2) 슬래시 커맨드 실행 결과 산출물, (3) 테스트에서 발견한 문제와 수정 내역 한 단락, (4) 이 스킬에서 "사람의 확인 지점"이 어디인지 한 문장. 스스로 확인할 것은 절차의 구체성과 확인 지점 설계의 설득력이다.
