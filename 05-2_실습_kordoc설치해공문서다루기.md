...
**과제 5-B. 공시 서식 3,103장을 보고서 한 장으로.** 2.8의 과제다. 제출물은 셋이다. (1) 양식대로 만든 보고서 hwpx, (2) 법인별 집계표 CSV와 계산에 쓴 코드, (3) 어떤 스킬을 설치해 어디에 썼는지와, 설치 전에 무엇을 확인했는지 적은 한 문단. 첫 제출본은 2.8의 채점 기준 넷을 채우면 통과다. 통과한 뒤에는 같은 보고서의 품질을 한 단계 올려 다시 낸다. 품질을 올리는 방향은 자유롭게 고르되 예를 들면 이렇다. 요약 상자를 과장님이 30초에 읽을 수 있게 다듬는 것, 상위 법인 표에 총자산과 수익을 나란히 두어 순위가 항목마다 다름을 보이는 것, 문제점 절의 개수마다 세는 방법을 붙이는 것, 표의 서식이나 결재란처럼 문서의 꼴을 과의 양식에 더 가깝게 하는 것이다. 두 번째 제출본에는 무엇을 왜 바꾸었는지 한 문단을 붙인다. 평가 기준은 첫 제출본은 채점 기준 넷의 충족 여부이고, 두 번째 제출본은 바꾼 것이 실제로 읽는 사람의 부담을 줄였는지, 그리고 바꾸는 과정에서 숫자가 어긋나지 않았는지다.
