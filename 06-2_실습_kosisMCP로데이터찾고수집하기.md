...
1회차에서 공공데이터의 지도(어디에 있나), 받기 전에 읽을 것(형식·메타데이터·이용허락), 그리고 에이전트에 도구를 꽂는 표준(MCP)을 배웠다. 이번 실습은 그 지도를 들고 실제로 길을 나선다. 1회차에서 배운 개념은 그것이 필요한 단계에서 화면에 나온 결과를 놓고 다시 짚는다. 단계마다 있는 "먼저 알아 둘 것"과 "짚고 가기"가 그 자리다. 전반부에서는 포털과 에이전트의 웹 검색으로 데이터를 찾는 연습을 하고, 후반부에서는 kosis MCP 서버를 주소 한 줄로 등록해 에이전트가 KOSIS 통계를 직접 검색·수집하게 만든다. 수집은 같은 통계표에서 두 번 한다. 2023년 시도별 총인구(한 시점, 여러 지역)와 전국 총인구의 2019-2023년 추이(한 지역, 여러 시점)다. 수집한 값은 포털 화면과 대조해 검증하고, CSV와 수집기록으로 남기며, 4주차에 만든 csv-profile 스킬로 첫 진단까지 마친다. 마지막으로 두 번 반복한 수집 절차를 4주차에서 배운 방식대로 스킬 하나에 굳히고, 그 스킬을 새 조건으로 한 번 돌려 시도별 다섯 해 치를 한꺼번에 받는다. 다음 데이터부터는 통계표와 조건만 말해 주면 같은 순서가 그대로 돌아간다.
...
- `data/korea_pop_2019_2023.csv`: 전국 주민등록 총인구의 2019-2023년 추이
- `수집기록.md`: 언제, 어디서, 어떤 조건으로 받았고 어떻게 검증했는지의 기록 (수집 2건)
- `산출물/프로파일_sido_pop_2023.md`: 4주차 스킬로 만든 첫 진단 보고서
- `.claude/skills/kosis-collect/`: 오늘의 수집 절차를 굳힌 스킬 (SKILL.md와 참조 파일)
- `data/sido_pop_2019_2023.csv`: 그 스킬로 받은 시도별 총인구 2019-2023년 (17개 시도 곱하기 5개 연도, 85건)
- 학기 프로젝트의 주제와 데이터 결정 (주제 한 문장, 분석 질문, 쓸 데이터)
...
**짚고 가기: 후보 표의 포털 열이 공공데이터의 지도다.** 방금 나온 후보 표에서 세 번째 열을 보라. 같은 주제의 데이터인데 출처가 KOSIS, 지방재정365처럼 서로 다른 포털로 갈린다. 한국의 공공데이터가 한 곳에 모여 있지 않기 때문이고, 흩어져 있는 이유는 무질서가 아니라 데이터의 성격이다. 통계는 통계를 만드는 기관이, 재정은 재정을 관리하는 기관이 가장 정확한 원천이다. 그래서 무엇이든 일단 찾아보는 종합 창구(공공데이터포털)가 하나 있고, 주제가 분명할 때 바로 가는 전문 창구들이 따로 있다. 공식 통계 수치가 필요하면 KOSIS, 지자체 살림살이가 궁금하면 지방재정365, 거시경제 지표는 한국은행 ECOS, 공공기관 경영정보는 ALIO, 근거 조문은 국가법령정보센터, 개방되지 않은 자료를 요청하려면 정보공개포털이다.

여러분이 브라우저로 직접 비교해 본 두 포털의 성격 차이도 이 구조에서 나온다. 공공데이터포털이 기관들이 내놓은 것을 모아 둔 시장이라면, KOSIS는 품질 관리를 거친 공식 통계의 서고에 가깝다. 오늘 후반부에서 KOSIS를 수집 대상으로 삼는 이유가 여기에 있다. 포털 화면과 메뉴 이름은 수시로 바뀌므로 외울 것은 클릭 순서가 아니라 "이런 성격의 데이터는 이런 성격의 기관이 낸다"는 구조다. (자세한 설명은 1회차 1.1, 표 6-1)

**검증.** 에이전트가 준 링크를 하나도 빠짐없이 브라우저에서 직접 클릭해 열어 보고 (1) 정상 (2) 페이지는 열리지만 내용이 설명과 다름(예: 시군구 단위라고 했는데 실제로는 시도 단위) (3) 열리지 않거나 엉뚱한 곳으로 감으로 분류한다. 웹 검색 결과를 조합하는 과정에서 에이전트는 오래된 주소를 가져오거나 그럴듯한 주소를 지어내기도 하므로, 2주차에서 본 "확인할 수 없는 답"이 여기서는 링크의 형태로 나타난다. (2)나 (3)이 하나라도 나오면 그 후보는 목록에서 빼거나 데이터셋 이름으로 포털에서 직접 다시 찾는다. 이런 불확실성이야말로, 잠시 뒤 MCP처럼 원천에 직접 연결된 통로가 필요한 이유이기도 하다.
...
**먼저 알아 둘 것.** 서버를 등록하는 일은 에이전트에게 새 손발을 달아 주는 일이므로, 주소를 입력하기 전에 세 가지를 점검하고 들어간다. ① 운영 주체가 확인되는가. 오늘 꽂을 서버는 수업에서 안내한, 국가통계포털(KOSIS) 기반으로 운영되는 실습용 시범서비스다. ② 무엇을 할 수 있는 서버인가. 통계를 검색하고 조회하는 읽기 전용 도구들이고, 내 파일을 고치거나 지우는 능력은 없다. ③ 꼭 필요한가. 오늘 수집 실습의 핵심 도구다. 이 점검을 건너뛰면 안 되는 이유는 검증되지 않은 서버가 잘못된 데이터를 주거나, 밖에서 가져온 내용 속에 숨긴 지시문으로 에이전트를 조종하려 들 수 있기 때문이다. 세 점검을 통과했으니 꽂는다. (자세한 설명은 1회차 1.5)
...
**짚고 가기: 방금 화면에 늘어난 도구 목록이 MCP가 한 일이다.** 지금 `/mcp` 화면 아래쪽에 보이는 네 줄이 오늘 등록으로 생긴 변화의 전부다. 어제까지 에이전트가 쓸 수 있던 도구는 내 노트북 안의 파일과 터미널, 그리고 웹 검색이었다. 거기에 통계 검색, 지역 통계 검색, 통계표 구조 확인, 통계값 조회라는 네 개가 나란히 더해졌다. MCP(Model Context Protocol)는 에이전트를 외부 도구·데이터베이스·서비스와 연결하기 위한 개방형 표준이고, 이 표준을 따르는 연결 통로를 MCP 서버라고 부른다. 콘센트 규격에 빗대면 이해가 빠르다. 규격이 통일되어 있으면 어떤 기기든 꽂으면 그만이듯, MCP 규격을 따르는 서버는 무엇이든 주소 한 줄로 꽂힌다.

![그림 6-2. MCP: 에이전트와 외부 시스템을 잇는 공통 규격](figures/fig06_mcp.png)

그림 6-2를 지금 화면과 맞춰 읽어 보자. 왼쪽의 에이전트가 여러분의 Claude Code이고, 가운데의 MCP가 방금 `claude mcp add`로 연결한 규격이며, 오른쪽 맨 위의 통계 데이터베이스(KOSIS 등)가 kosis 서버가 닿아 있는 곳이다. 오른쪽의 다른 상자들, 곧 업무 시스템이나 메일·저장소 같은 외부 서비스도 같은 방식으로 꽂힌다. 여기서 알 수 있는 것은 에이전트 입장에서 오른쪽에 무엇이 꽂히든 대화 방식이 같다는 점이다. 그래서 잠시 뒤 여러분은 KOSIS의 인증키를 발급받거나 API 문서를 읽지 않고도, 파일을 읽어 달라고 말하듯 통계를 받아 달라고 말하게 된다. 다만 오른쪽에 꽂히는 것이 읽기 전용 통계 조회일 수도, 파일을 지우거나 메일을 보내는 서버일 수도 있다는 점이 등록 전 신뢰 점검을 세 가지나 거친 이유다. (자세한 설명은 1회차 1.4)
...
**검증.** 에이전트가 실행한 명령의 이름(kosis)과 주소가 지시한 그대로인지 읽고, 재시작 후 `/mcp` 화면에서 상태가 연결됨이고 검색·구조 확인·조회 도구가 보이는지 확인한다. 범위가 옮겨졌는지는 다른 폴더를 하나 열어 `/mcp`를 눌러 보는 것이 가장 확실하다. 마지막으로 등록 명령의 의미를 자기 말로 설명할 수 있는지 점검한다. `kosis`는 내가 붙인 별명이고, 주소는 서버의 위치이며, 이 서버는 읽기 전용 통계 도구를 제공한다. 무엇을 꽂았는지 모른 채 꽂는 것이 1회차에서 경고한 가장 나쁜 습관이다.
...
여기서 읽을 것이 셋 있다. 첫째, 항목이다. 총인구수 말고 성별 항목도 있으므로, 조회할 때 총인구수를 지정하지 않으면 남자·여자 값이 섞여 올 수 있다. 둘째, 분류의 계층이다. 표 이름에 "시군구"가 들어 있듯 이 표는 전국, 시도, 시군구를 한 계층 구조로 담고 있고, 우리는 그중 시도 수준만 원한다. 다음 단계에서 이 계층이 실제로 말썽을 일으킨다. 셋째, 기준이다. 주민등록 기준 인구는 인구총조사 기준 인구와 다르다.

**짚고 가기: 아직 숫자는 한 개도 오지 않았다.** 구조 확인 도구가 돌려준 위의 [구조] 세 줄을 다시 보라. 통계값은 하나도 없고 항목 이름, 분류의 계층, 수록 기간, 단위만 있다. 이것이 메타데이터이고, 숫자보다 먼저 읽어야 하는 것이다. 약국에서 약을 받으면 약봉투부터 읽듯이, 데이터에도 먼저 읽을 설명서가 붙어 있다.
...
<strong>정의 · 메타데이터와 코드북</strong><br>
<strong>메타데이터</strong>(metadata)는 데이터 자체가 아니라 데이터에 대한 설명 정보다. 누가 만들었고(생산 기관), 언제 기준이며(기준 시점), 얼마나 자주 갱신되고(갱신 주기), 단위가 무엇인지 등이 담긴다. 포털의 데이터셋 상세 화면에 표 형태로 제시되는 것이 보통이다.<br>
<strong>코드북</strong>(codebook)은 그중에서도 각 열(변수)의 정의를 적은 문서다. 열 이름이 무슨 뜻인지, 값이 어떤 코드로 적혀 있는지(예: 성별 1=남, 2=여), 어떤 산식으로 계산되었는지를 알려 준다.
</div>

왜 숫자보다 먼저인지는 위 결과의 "항목: 총인구수(T20)"만 보아도 드러난다. T20이라는 코드가 무엇을 세는 것인지는 값 안에 없고 표의 설명에 있다. 이 표가 내 질문에 맞는지는 여섯 항목으로 따진다. ① 생산 기관과 원천(같은 "인구"라도 주민등록 기준과 인구총조사 기준은 값이 다르다), ② 기준 시점, ③ 단위와 산식, ④ 범위(내가 원하는 지역이 모두 있는가, 원하지 않는 계층이 섞이지는 않는가), ⑤ 갱신 주기와 이력(과거 값이 소급 수정되기도 하는가), ⑥ 이용허락과 출처 표시다. 앞의 다섯은 데이터가 내 질문에 맞는지를 묻고, 마지막 하나는 그 데이터를 내 보고서에 써도 되는지를 묻는다. 우리 공공기관은 개방 저작물에 공공누리(KOGL)라는 표준 이용허락 표시를 붙이는데, 네 유형 모두 출처 표시가 공통 의무이고 유형에 따라 상업적 이용과 변경이 갈린다. 수업 과제는 상업적 이용이 아니므로 제약은 드물지만 출처 표시에는 예외가 없다. 여섯 항목이 어긋난 채 분석하면 코드가 완벽해도 결론이 틀린다. 표 6-4가 저자의 판정 결과다. (자세한 설명은 1회차 1.2, 표 6-3)
...
**검증.** 브라우저에서 KOSIS의 통계표 "행정구역(시군구)별 성별 인구수"를 열어, MCP가 알려 준 구조(항목 셋, 전국·시도·시군구 계층, 수록 기간)가 화면의 선택 창과 같은지 확인한다. 표 6-4의 마지막 두 항목인 갱신 주기와 이용허락은 MCP 응답에 없으므로 이때 화면의 수록 정보에서 함께 읽어 둔다. 표 이름에 "주민등록"이 없거나 판정표의 첫 행이 어긋나면 에이전트가 인구총조사 계열의 표를 고른 것이므로 "주민등록 기준의 표로 다시 골라 줘"라고 재지시한다. 표를 잘못 고르면 이후의 모든 숫자가 정확하면서도 틀린 답이 된다.
...
표 아래에 통계표명과 KOSIS 링크가 붙고, 응답 첫머리에는 아래 유의 박스의 안내문이 붙는다.

**짚고 가기: API의 우회로와 MCP의 직통로.** 방금 한 일을 되짚어 보면 하지 않은 일이 더 눈에 띈다. 인증키를 발급받지 않았고, API 문서를 읽지 않았고, 수집 코드를 짜지 않았다. 원래 프로그램이 다른 시스템의 데이터를 받아 오는 문은 API(application programming interface)였다. 웹페이지가 사람용 문이라면 API는 프로그램용 문으로, 화면 장식 없이 데이터만 약속된 형식으로 오간다. KOSIS도 오픈API를 제공하므로 전통적인 방법으로도 이 값을 받을 수 있지만, 그러려면 키를 발급받아 관리하고, 문서를 읽어 파라미터 이름을 알아내고, 요청 주소를 조립하는 코드를 짜서 고쳐 가며 돌려야 한다. 조회 조건이 코드에 글자로 남아 재현성은 해결되지만 준비물이 많은 우회로다.

MCP는 그 우회로를 없앤다. 위 로그의 네 줄(통계표, 항목, 분류, 기간)이 원래는 사람이 문서를 읽어 가며 조립해야 했던 요청의 알맹이인데, 지금은 에이전트가 도구에 그대로 넘기는 조건이 되어 있다. 문서를 주는 대신 도구를 꽂았기 때문이다. 대신 사라지지 않는 것이 하나 있다. 조건이 맞는지, 값이 맞는지 판정하는 일이다. 에이전트가 도구를 골라 쓰는 것을 지켜보고 결과를 검증하는 것이 사람의 일이며, 그 일을 바로 아래에서 한다. (자세한 설명은 1회차 1.3, 1.4)
...
② 건수가 설명되는지 본다. 전국 1건 + 시도 17건 = 18건인가. 시도가 15개나 20개로 나왔다면 어딘가 새거나 겹친 것이다. 다만 17이라는 수 자체도 고정된 값은 아니다. 시도의 수는 행정구역 개편으로 달라질 수 있으므로, 건수 기준을 세울 때는 통계표 화면의 시도 목록을 먼저 세어 조회하는 시점에 값이 실린 시도가 몇 곳인지 확인한다. 이 단계의 가장 흔한 틀림이 여기서 드러난다. 2.3에서 본 분류의 계층 때문에, 시도 수준을 따로 지정하지 않으면 시군구 행이 함께 따라온다. 화면에는 전국, 서울특별시 다음 줄부터 "서울특별시 종로구", "서울특별시 중구"가 이어지고 표가 수백 행으로 늘어난다. 에이전트는 이것을 실패로 여기지 않는다. 조회는 성공했고 값도 모두 진짜이기 때문이다. 틀린 것은 값이 아니라 범위이고, 그것을 알아채는 것은 "18건이어야 한다"는 기준을 미리 세워 둔 사람뿐이다. 발견했으면 다음과 같이 재지시한다.
...
**첫째, 값이 포털 화면과 같은가.** 같은 데이터로 통하는 두 개의 문을 검증에 활용한다. MCP라는 문으로 받은 값을, 사람용 문인 KOSIS 웹 화면으로 대조하는 것이다. 2.3에서 열어 둔 통계표로 돌아가 항목은 총인구수만, 시점은 2023년만, 행정구역은 전국·서울특별시·세종특별자치시 셋만 남기고 조회한 뒤, 화면의 세 값을 에이전트가 받은 값(전국 51,325,329명, 서울 9,386,034명, 세종 386,525명)과 자릿수까지 비교한다. 이때 표에 적힌 단위 표기(명)와 수록 시점 표기도 함께 읽어 둔다. 값만 맞추고 단위를 흘려 읽으면 뒤에서 숫자가 천 배로 어긋난다.

세 값을 고른 데는 이유가 있다. 전국은 곧이어 시도 합계와 맞춰 볼 기준값이고, 서울은 자릿수가 커서 한 자리만 어긋나도 눈에 띄며, 세종은 가장 작아 다른 지역과 헷갈릴 일이 없다. 셋이 모두 맞으면 나머지 열네 곳도 같은 조회에서 함께 온 값이므로 표본 대조로 전체를 갈음할 수 있다. 반대로 셋 중 하나라도 어긋나면 값 하나의 문제가 아니라 조회 조건의 문제일 가능성이 크다. 그럴 때는 값을 고칠 것이 아니라 어느 조건이 달랐는지부터 찾아야 한다. 화면 위쪽의 항목·행정구역·시점 선택 영역이 MCP 조회에서 도구에 넘긴 네 줄의 조건과 하나씩 짝을 이룬다는 점을 여기서 확인해 두면, 앞으로 조회 조건을 지시할 때 무엇을 적어야 하는지가 분명해진다.
...
**예상되는 에이전트의 행동과 결과.** 에이전트가 짧은 파이썬 코드를 만들어 uv 환경에서 실행한다. 도구 사용 로그에는 MCP 호출 대신 코드 실행과 파일 쓰기가 나타난다. 이미 받은 값을 다루는 일이므로 새 조회는 없다. 대화창에는 합계와 전국 값의 비교, 파일의 처음 5행, 그림 저장 보고가 순서대로 온다. 저자의 실행에서 17개 시도의 합은 51,325,329명으로 전국 값과 정확히 일치했다. 저장 형식을 CSV로 고른 것은 첫 줄에 열 이름을 쓰고 그다음 줄부터 값을 쉼표로 구분해 적은 텍스트 파일이라 어떤 프로그램에서든 오해 없이 읽히기 때문이다. 대신 CSV에는 인코딩이라는 주의할 점이 하나 붙고, 지시문에 적어 준 utf-8-sig가 엑셀에서 한글이 깨지지 않게 하는 그 지정이다(1회차 1.2, 표 6-2). 저장된 CSV의 첫 다섯 행은 다음과 같았다.
...
**짚고 가기: 이 표가 없으면 셋째 달에 숫자가 어긋난다.** 방금 만든 수집기록이 왜 그토록 자세한지는 이 일을 매달 반복한다고 생각해 보면 드러난다. 첫 달은 할 만하다. 통계표를 찾고, 조회 조건을 맞추고, 값을 받아 보고서에 옮긴다. 둘째 달에는 지난달에 조건을 어떻게 맞췄는지 헷갈리기 시작한다. 셋째 달에는 지역 수준이나 항목을 지난달과 다르게 골라, 두 달의 숫자가 서로 비교할 수 없는 것이 되어 버린다. 문제는 성실성이 아니라 기록이다. 클릭이든 대화든 손으로 하는 수집은 어디에도 조건이 남지 않으므로 반복하면 반드시 어긋난다. 표 6-6의 "조회 조건" 한 칸이 그 어긋남을 막는 장치다.

재현성(reproducibility)은 같은 절차로 같은 결과를 다시 만들 수 있는 성질이고, 공공부문에서는 학술적 미덕이기 이전에 행정의 연속성 문제다. 공공기관의 분석 결과는 예산과 정책의 근거가 되고 감사와 정보공개청구의 대상이 되므로, "작년 보고서의 이 숫자는 어떤 조건으로 뽑은 것인가"에 답하지 못하면 숫자의 신뢰가 무너진다. 담당자가 바뀌어도 마찬가지다. 수집 절차가 기록으로 남아 있으면 후임자는 실행만 하면 되지만, 전임자의 손과 기억에만 있었다면 처음부터 다시 시작해야 한다. 오늘 표 6-6을 채우는 몇 분이 그 차이를 만든다. (자세한 설명은 1회차 1.3)

**검증.** data/sido_pop_2023.csv를 열어 헤더 1행 + 데이터 17행인지, 한글이 멀쩡한지, 표 6-5의 첫 행(경기도, 13630821)이 그대로 있고 총인구 값에 쉼표나 따옴표가 섞여 문자열이 되지 않았는지 본다. 수집기록.md은 모든 칸이 채워졌는지, 특히 검증 내용이 자기 문장으로 적혔는지 확인한다. 마지막으로 새 대화를 열고 수집기록에 적어 둔 지시문으로 처음부터 다시 수집해, 같은 18건과 같은 값이 나오는지 본다. 이것이 2주차에서 배운 새 대화 독립 검산이며, 13주차에서는 이 일을 검증 전담 서브에이전트(verifier)에게 맡긴다. 그때 서브에이전트에게 건네는 지시서의 재료가 오늘의 수집기록이다.

## 2.6 두 번째 수집: 전국 총인구 2019-2023년 추이와 손 검산

**무엇을 왜 하는가.** 같은 표에서 조회 조건만 바꿔 한 번 더 수집한다. 지역을 전국 하나로 고정하고 시점을 2019-2023년 다섯 해로 늘린다. 두 번째 수집을 하는 이유는 셋이다. 절차는 두 번째부터 손에 붙는다는 것, 시계열 수집에는 기간 지정이라는 고유한 함정이 있다는 것, 그리고 이 다섯 값을 12주차에서 "우리나라 인구는 감소 추세다"라는 문장의 근거 메모에 다시 쓴다는 것이다. 2.5에서 짚은 갱신 문제도 여기서 실감할 수 있다. 조회 조건을 기록해 두면 내년에 2024년 값을 덧붙이는 일은 지시문의 숫자 하나를 바꾸는 일이 된다.
...
**예상되는 에이전트의 행동과 결과.** 에이전트가 SKILL.md의 절차를 따라 파일 존재를 확인하고, 코드로 계산하고, `산출물/프로파일_sido_pop_2023.md`와 `code/profile_sido_pop_2023.py`를 저장했다고 보고한다. 보고서는 4주차에서 정한 다섯 절의 형식을 따르며, 대략 다음 내용이 담긴다.
...
**검증.** 스킬의 산출물도 검증 대상이다. 4주차의 유의 박스가 경고했듯, 형식이 말끔할수록 안을 들여다보는 습관이 무뎌진다. 보고서의 행 수(17)와 최댓값(13,630,821)이 2.5에서 확인한 파일과 같은지 보고, 평균은 손으로 검산한다. 17개 시도의 합이 전국 값과 같음을 2.5에서 확인했으므로 평균은 51,325,329 ÷ 17이고, 계산기로 나누면 3,019,137로 나누어떨어진다. 경고에 대한 판단은 보고서의 "5. 확인 필요" 절 아래에 자기 문장으로 적는다. "연도 열의 단일값 경고는 2023년만 수집한 설계의 결과이므로 문제 아님"이 한 예다.
...
**예상되는 에이전트의 행동과 결과.** 폴더 하나와 파일 두 개가 생겼다는 보고가 온다. 대화창에 `/`를 치면 자동완성 목록에 `/kosis-collect`와 머리말에 적어 둔 설명이 보인다. 새 대화를 열어 다음과 같이 부른다. 스킬 이름을 적은 뒤, 이번에 쓸 조건을 같은 줄에 한 문장으로 붙이면 된다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시</strong><br>
"/kosis-collect 통계표 DT_1B040A3에서 2019년부터 2023년까지 17개 시도의 총인구를 받아 줘. 지역은 시도 수준으로만 한정해 줘."
</div>

스킬 이름을 대지 않고 "DT_1B040A3에서 2019-2023년 시도별 총인구를 kosis로 받아 CSV와 수집기록으로 남겨 줘"처럼 평범한 문장으로 부탁해도 된다. 4주차에서 본 대로 머리말의 `description`이 자동 호출의 판단 근거이기 때문이다. 어느 쪽으로 부르든 에이전트가 SKILL.md의 절차를 순서대로 밟는 모습이 도구 사용 로그에 나타난다. 아래는 그 로그의 모양이다. 표시 방식은 버전에 따라 다르지만 읽을 것은 같다.

```text
[스킬] kosis-collect   받은 조건: 통계표 DT_1B040A3 / 기간 2019-2023 / 지역 시도 수준
[도구 호출] kosis_table_info   통계표: DT_1B040A3
[결과] 항목 총인구수 외, 분류 행정구역(시군구)별, 수록 기간 ○○○○-○○○○, 단위 명
       기준 세우기: 시도 17개 지역에 5개 연도이므로 85건이 와야 한다
[도구 호출] kosis_get_data   통계표: DT_1B040A3 / 항목: 총인구수
                             분류: 행정구역(시군구)별 - 시도 수준 / 기간: 2019년 - 2023년
[결과] 85행
       서울특별시   2019   9729107
       서울특별시   2020   9668465
       서울특별시   2021   9509458
       ...
       건수 확인: 85건. 기준과 일치
[파일 쓰기] data/sido_pop_2019_2023.csv
[파일 쓰기] 수집기록.md   (검증 내용 칸은 비움)
```

부탁은 한 문장인데 화면에는 일곱 걸음이 다 지나간다. 2.4와 2.6에서 여러 번 주고받아야 했던 요구, 곧 항목을 하나로 좁히고 지역 수준을 한정하고 시작·끝 시점을 명시하고 건수를 세어 보고하는 일이 이번에는 요구하지 않아도 따라온다. 스킬에 적어 둔 것이 문장이 아니라 판단이기 때문이다. 다만 절차가 짧아 보이는 것과 검증이 줄어드는 것은 다른 이야기다. 파일이 저장되었다는 보고는 아직 검증이 아니다.

**짚고 가기: 85라는 숫자가 클릭 수집과 갈리는 자리다.** 방금 받은 것은 2.5의 18건보다 네 배 넘게 많은 85건이다. 이만큼을 포털 화면에서 손으로 받으려면 지역 열일곱 곳과 다섯 해를 골라 조회한 다음, 화면의 숫자 여든다섯 개를 파일로 옮겨 적어야 한다. 옮겨 적다가 한 자리를 잘못 눌러도 화면은 아무 말을 하지 않는다. 손으로 하는 수집이 반복하면 반드시 어긋나는 이유가 여기에 있다. 자동 수집은 그 옮겨 적는 단계를 통째로 없앤다. 값이 도구에서 파일로 바로 가므로 사람이 할 일은 옮겨 적기가 아니라 몇 건이 와야 하는지 미리 정해 두는 일 하나로 줄어든다. SKILL.md의 절차 2가 곱셈 한 줄로 되어 있는 이유이며, 받는 양이 커질수록 이 한 줄이 눈으로 세는 확인을 대신한다. (자세한 설명은 1회차 1.3)

**검증.** 두 가지로 확인한다.

① 산출물 대조. 새 파일의 2023년 행에서 경기도 13,630,821, 서울특별시 9,386,034, 세종특별자치시 386,525를 찾아 2.5에서 저장한 `data/sido_pop_2023.csv`의 같은 값과 맞춰 본다. 연도마다 17개 시도를 더해 2.6의 전국 값과 견주면 확인이 한 겹 두꺼워진다. 2019년의 합이 51,849,861이면 그해 열일곱 값이 모두 제자리에 있다는 뜻이다.

② 건수와 기록. 화면에 적힌 건수 기준이 85이고 실제 수신 건수도 85인지 본다. 다르면 지역 수준이 시군구까지 내려갔거나 기간이 잘린 것이고, 그때는 스킬이 절차 4에서 멈춰 원인부터 보고해야 한다. 스킬이 새로 쓴 수집기록 항목에 앞의 두 기록과 같은 항목이 다 있는지, 검증 내용 칸이 비어 있는지도 확인한다. 채워져 있다면 절차 6의 마지막 문장을 어긴 것이므로 그 문장을 더 분명하게 고친다.
...
- [ ] 등록을 에이전트에게 지시했고, 실행된 명령의 이름·주소가 지시와 일치하며 재시작 후 `/mcp`에서 kosis가 연결됨 상태로 도구 목록과 함께 보이는 것을 확인했다
- [ ] 등록 범위를 확인해 사용자 범위로 옮겼고, 다른 폴더를 열어도 `/mcp`에 kosis가 보이는 것을 확인했다
- [ ] 2.3에서 고른 통계표의 항목·분류·기간을 KOSIS 화면과 대조했고, 화면에서만 알 수 있는 갱신 주기와 이용허락도 읽어 두었다
- [ ] 수집 대화의 도구 사용 로그에서 kosis 도구 호출을 확인했다 (기억으로 답하지 않았다)
- [ ] 수신 건수(18)가 전국 1 + 시도 17로 설명됨을 확인했다 (시군구 행이 섞였다면 재지시로 바로잡았다)
- [ ] 전국 51,325,329, 서울 9,386,034, 세종 386,525를 KOSIS 화면에서 찾아 자릿수까지 대조했고, 화면의 단위 표기와 시점 표기도 함께 보았다
- [ ] CSV 열 이름, 그림 축 라벨, 요약 문장의 단위(명·천 명·만 명)가 서로 어긋나지 않는지 확인했다
- [ ] 17개 시도 합계가 전국 값(51,325,329)과 일치하고, CSV가 17행이며 총인구 열이 숫자이고 한글이 깨지지 않음을 확인했다
...
- [ ] 2.8의 kosis-collect 스킬을 만들어 시도별 2019-2023년 수집에 실행했고, 건수 기준(17 곱하기 5 = 85)과 실제 수신 건수가 같으며 2023년 값이 `data/sido_pop_2023.csv`와 일치함을 확인했다
- [ ] 스킬이 새로 쓴 수집기록 항목이 앞의 두 기록과 같은 항목으로 채워졌고, 검증 내용 칸은 비어 있음을 확인했다
...
**혼자 해 보기 6-A. MCP 수집·검증 세트.** 오늘 만든 두 파일(data/sido_pop_2023.csv, 수집기록.md)을 폴더에 남겨 둔다. 스스로 확인할 것은 세 가지다. ① 수집기록의 지시문만으로 같은 데이터를 다시 수집할 수 있는가, ② 두 겹 검증(화면 대조, 시도 합계와 전국 값 비교)이 수집기록에 구체적으로 적혔는가, ③ 출처(통계표명, 작성 기관)가 정확히 기재되었는가.

**혼자 해 보기 6-B. 내 프로젝트 데이터로 반복.** 오늘 정한 학기 프로젝트 데이터에 같은 절차를 적용한다. 프로젝트 데이터가 KOSIS에 있다면 kosis MCP로 수집하고, 다른 포털에 있다면 파일로 내려받는다. 어느 쪽이든 data 폴더에 저장하고 수집기록.md에 항목을 추가하며, `/csv-profile`로 첫 진단 보고서까지 만든다. 수집기록에는 "이 데이터가 내 프로젝트의 어느 질문에 쓰이는가"를 한 문단 덧붙인다.

**혼자 해 보기 6-C. 추이 수집과 손 검산 세트.** 2.6과 2.7의 산출물을 한자리에 묶어 둔다. (1) data/korea_pop_2019_2023.csv, (2) 손으로 채운 표 6-7과 에이전트의 증감 계산을 대조한 결과, (3) 수집기록.md의 두 번째 항목, (4) 산출물/프로파일_sido_pop_2023.md와 그 "확인 필요" 절에 적은 자신의 판단 문장. 스스로 확인할 것은 손 검산의 정확성과, 수집 중 재지시가 있었다면 그 원인(기간 누락, 시군구 섞임, 안내문 누락 등)을 기록으로 남겼는가이다. 재지시가 한 번도 없었다면 "왜 없었는가"를 지시문의 손잡이와 연결해 한 문단으로 쓴다.

**혼자 해 보기 6-D. 수집 절차의 스킬화.** 2.8에서 만든 `.claude/skills/kosis-collect/` 폴더(SKILL.md와 참조 파일)와 그 스킬로 수집한 `data/sido_pop_2019_2023.csv`, 그리고 스킬이 덧붙인 수집기록 항목을 한자리에 모아 둔다. 스스로 확인할 것은 둘이다. (1) 무엇을 절차에 고정했고 무엇을 부를 때 말로 일러 주기로 했는지, 그 기준을 자기 문장 한 문단으로 설명했는가. (2) 파일이 85행이고, 2023년 값이 혼자 해 보기 6-A의 파일과 일치하며, 연도별 시도 합계가 2.6의 전국 값과 맞는가. 이 스킬은 14주차 종합 프로젝트에서 자신의 데이터에 그대로 쓰게 되므로, 실습을 마친 뒤에도 폴더에 남겨 둔다.
