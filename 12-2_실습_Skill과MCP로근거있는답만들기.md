...
**심화. 세 방향으로 스킬을 키운다.** 초안을 다시 읽으면 아쉬운 데가 셋 보인다. 첫째, 문서 폴더가 본문에 `data/laws`로 못박혀 있다. 다른 폴더의 문서로 메모를 만들려면 그때마다 SKILL.md를 고쳐야 한다. 둘째, 표 12-3의 표기 규칙이 어디에도 없다. 본문에 넣으면 본문이 길어지고, 넣지 않으면 표기가 메모마다 흔들린다. 셋째, 산출물에 검산 기록을 적을 자리가 없다. 오늘 배운 대조를 매번 다른 파일에 적으면 메모와 검증이 흩어진다. 처방도 셋이다. 메모마다 달라지는 조건은 파일에 못 박지 말고 사용자가 말해 준 것을 따르게 하고, 매번 참조하는 규칙은 옆의 파일로 빼고, 판단이 필요 없는 대조는 스크립트에 맡긴다. 앞의 둘을 지금 적용하고 스크립트는 2.9에서 붙인다.
...
산출물이 여섯 절에서 일곱 절이 되었다. 새로 생긴 7절이 오늘의 핵심이다. 근거 메모와 그 메모를 검증한 기록이 한 파일에 함께 있으면, 나중에 이 메모를 읽는 사람이 "누가 언제 무엇을 확인했는가"까지 한자리에서 볼 수 있다. 사람만 쓸 수 있는 칸은 비워 두고 넘긴다.
...
**검증.** ① `/`를 입력했을 때 자동완성 목록에 `/evidence-memo`가 뜨고 머리말에 적어 둔 설명이 함께 보이는지 확인한다. ② 두 산출물의 파일 이름이 `산출물/근거메모_주차장.md`와 `산출물/근거메모_인구감소.md`인지, 메모 1절의 질문이 문장 전체로 적혀 있는지 본다. 파일 이름이 다르거나 질문이 한 낱말로 잘려 있으면 부탁하는 문장에서 주제와 질문이 또렷하지 않았던 것이므로, 둘을 분명히 갈라 적어 다시 부른다. ③ 도구 사용 로그에서 reference.md 읽기가 스킬 호출 직후가 아니라 표기 단계 근처에 나타나는지 본다. 참조 파일을 필요한 시점에만 꺼내 읽는 방식이 이 스킬에서도 작동하는지 보는 것이다. ④ 두 메모의 절 제목이 일곱 개이고 reference.md의 양식과 글자까지 같은지 본다. ⑤ 7절의 표가 빈 채로 왔는지 본다. 에이전트가 채워 놓았다면 "채우는 것은 사람이다"를 규칙에 다시 강조한다. ⑥ 두 메모의 "확인 필요" 절이 비어 있지 않은지 본다. 매번 비어 있다면 스킬이 판단을 사람에게 넘기지 않고 스스로 결정하고 있다는 뜻일 수 있다.
...
- [ ] `.claude/skills/evidence-memo/SKILL.md`가 스킬 목록과 `/` 자동완성에 나타나고, 머리말의 설명이 함께 보인다
- [ ] 스킬을 부르며 문서 폴더와 주제와 질문을 문장으로 적어 주었고, 메모 1절의 질문이 한 낱말이 아니라 그 문장 그대로 적혀 있다
- [ ] `reference.md`를 분리했고, 로그에서 그 파일이 표기 단계 근처에 읽히는 것을 확인했다
- [ ] `/evidence-memo` 실행 결과가 일곱 절 형식을 갖추고, 산출물 파일 이름에 내가 말한 주제가 들어갔다
- [ ] 7절의 검산 기록 표가 빈 채로 왔다 (에이전트가 미리 채우지 않았다)
- [ ] `scripts/cite_check.py`를 붙였고, 오늘 만든 메모로 실행해 인용 건수와 일치·불일치 요약을 확인했다
