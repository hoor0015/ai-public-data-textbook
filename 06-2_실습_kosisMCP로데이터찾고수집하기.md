...
VSCode에서 터미널을 열고(2주차에서 배운 그 터미널이다) 다음 한 줄을 입력한다.

<div style="border:1px solid #cfe3d4;border-left:5px solid #2f8f4e;background:#f4fbf6;padding:12px 16px;margin:18px 0;border-radius:4px;">
<strong>지시문 예시 (터미널 명령)</strong><br>
"claude mcp add --transport http kosis https://kosismcp2026.vercel.app/api/mcp"
</div>

**예상되는 결과.** 터미널에 서버가 추가되었다는 확인 메시지가 나온다. 이어서 Claude Code 대화창에 `/mcp`를 입력하면 등록된 서버 목록에 kosis가 보이고, 연결 상태와 이 서버가 제공하는 도구들(통계 검색, 통계값 조회, 통계표 구조 확인 등)을 확인할 수 있다.

**검증.** ① `/mcp` 화면에서 kosis 서버의 상태가 연결됨으로 표시되는지 확인한다. ② 도구 목록에 검색과 조회에 해당하는 도구가 보이는지 훑어본다. ③ 마지막으로 등록 명령의 의미를 자기 말로 설명할 수 있는지 점검한다. `kosis`는 내가 붙인 별명이고, 주소는 서버의 위치이며, 이 서버는 읽기 전용 통계 도구를 제공한다. 무엇을 꽂았는지 모른 채 꽂는 것이 1회차에서 경고한 가장 나쁜 습관이다.
...
- [ ] `/mcp` 화면에서 kosis 서버가 연결됨 상태이고 도구 목록이 보이는 것을 확인했다
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 터미널에서 `claude` 명령을 찾을 수 없다고 나온다 | Claude Code CLI가 설치되지 않았거나 터미널을 새로 열지 않음 | 1주차 설치 절차를 다시 확인하고, 설치 후에는 터미널을 새로 연다 |
| `/mcp`에 kosis 서버가 안 보인다 | 등록 명령을 다른 폴더에서 실행했거나 오타 | 프로젝트 폴더의 터미널에서 등록 명령을 다시 실행. 주소 철자를 그대로 복사해 붙인다 |
